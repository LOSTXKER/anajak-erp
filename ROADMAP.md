...
- ทำถึงไหน: 2026-09-14 เบสสั่งรื้อ UX/UI ใหม่แนว minimal modern (Notion/Vercel) มุมโค้ง มีลูกเล่น คงน้ำเงิน Anajak และฟอนต์ Prompt สีอื่นรื้อได้ ทำหน้าแรกเป็นต้นแบบก่อน. ต้นแบบ = ไฟล์ HTML เดี่ยวในสมอง `records/projects/anajak-erp/mockup-home-minimal-2026-09-14.html` (artifact `claude.ai/code/artifact/b29b1705-5303-4d8f-b99d-af7e879d485e`) มีสวิตช์เทียบแบบเดิมและ 4 มุมมอง; โค้ดแอปยังเท่า `7691bf7`. เบสเคาะทิศ "ใช่เลย" → ปัด "ต้องเช็กก่อน" แบบตัวเลขนับ → รอบ 2 รายการออเดอร์ที่ต้องจัดการ → เบสขอให้การ์ดซ้ายบนเป็นเรื่องโรงงาน (สุขภาพ/flow/การเชื่อมต่อ node) → รอบ 3 มี 2 แบบ: ก ผังไหล (node จริงจาก `factory-station.ts`/`production-steps.ts`: ออกแบบ → เตรียมเสื้อ/พิมพ์ฟิล์ม DTF/ร้านนอก → รีดร้อน (รอฟิล์ม∧เสื้อ) → QC → แพ็ก → ส่ง พร้อมคอขวด) / ข กระดานสุขภาพ (ข้อเท็จจริง 4 ช่อง + แถวต่อ node แถบงานในมือ/กำลังต่อวัน); รายการต้องจัดการเป็นคอลัมน์ในตารางออเดอร์
- ตรวจแล้ว: ต้นแบบรอบ 3 เปิดจริงบน 1440 และ 375 ทั้งสว่าง/มืด ไม่ล้นแนวนอน (ผังเลื่อนในการ์ดเองบนมือถือ); ก/ข สลับได้ เส้นเชื่อมวาดตามตำแหน่งจริงและวาดใหม่เมื่อย่อจอ มุมมองไม่เห็นเงิน/โรงงานปกติ/กำลังโหลดทำงาน. รอบนี้ไม่แตะโค้ดแอป จึงไม่มีผล typecheck/test ใหม่
- ติดอะไร: รอเบสเลือก ก หรือ ข; ผัง/กระดานต้องมี query ใหม่ (นับใบงานต่อสถานีจาก `factory.board`, ด่านรีดร้อนรอฟิล์ม/เสื้อจาก `evaluateHeatPressGate`, งานร้านนอกเลยรับ, กำลังต่อวันจาก WorkCenter ซึ่งยังว่างจนกว่า V2 cutover, ส่งตรงเวลา 7 วันตาม MFG1) ก่อนลงจริง; หน้าลอง `/proto` ในโค้ดถูกลบตั้งแต่ 09-11
- ทำต่อ: เมื่อเบสเคาะ → ลงหน้าแรกจริง (shell/เมนู/การ์ด/ตาราง) ให้ตรงต้นแบบทีละส่วน แล้วต่อหน้ารายละเอียดออเดอร์ด้วยชุด token เดียวกัน
