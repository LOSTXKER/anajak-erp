...
- ทำถึงไหน: main = `e875e24` (ขึ้นเว็บจริง 2026-09-17 รวมฟอร์มออเดอร์รอบแรกที่ลงต้นแบบไม่ครบ) · **2026-09-17 เบสทัก "ดู local ไม่เห็นมีการแก้" → ตรวจพบรอบแรกลงตรงแค่ 57 จาก 182 จุด · เบสสั่ง "ทำให้ครบตามต้นแบบเลย"** · 2026-09-16 เบสสั่งทุกหน้าตามต้นแบบ 100% ยกเว้นออเดอร์/ผลิต โดยคงฟีเจอร์จริง → เทียบ 44 หน้า แก้ 353 จุด ขึ้นแล้ว
...
- ตรวจแล้ว (ฟอร์มออเดอร์): typecheck · lint 0 error (เตือน 17) · test 1865 ผ่าน · verify:ui ผ่านครบ (2 ด่านฟอร์มตรวจกฎเดิมบนคลาส kit) · วางภาพคู่ต้นแบบนอก Next ครบ 4 แท็บ × เปิดงาน/แก้ไข + หน้าว่าง + 500px + มืด · ลองกดแก้/เพิ่ม/ลบ/ส่งไม่ครบ · ตรวจอิสระรอบ 2: ตรง 110 คงกฎจริง 33 ที่เหลือต่าง ≤2px หรือเป็นชุดกลางทั้งเว็บ (หัวหน้า · DatePicker) · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน**
- ติดอะไร: ต้นแบบขัดข้อบังคับบางข้อจึงคงของจริงไว้ — เอกสารพิมพ์ (ต้นฉบับ/สำเนา · ลายน้ำยกเลิก · จำนวนเงินเป็นตัวอักษร · ส่วนลด · ใบลดหนี้/เพิ่มหนี้ · ใบแพ็คห้ามมีเงิน · ใบสั่งงานต้องมี QR+เวอร์ชันม็อกอัพ) และลิงก์ลูกค้า (ขอบเขตข้อมูลต่อ token · blind ship ปิดตัวตนร้าน) · ที่ยังไม่ทำเพราะต้องแตะ API/ด่านคู่กัน: ตัวเลขงานค้างของเมนู "งานของฉัน" · กล่องลอยแจ้งเตือนบนกระดิ่ง · หัวคอลัมน์ตารางพื้นเทา · โหมดมืดสลับให้รางสว่างกว่าพื้น · บั๊กเดิม: แถวตัดเย็บใหม่/ลูกค้าส่งมาที่ยังไม่ใส่ไซส์ นับ 1 ตัวและคิดเงินชั่วคราว (ส่งไม่ผ่านเพราะบังคับไซส์) · แตะตัวนับที่ใช้คิดเงิน รอเบสเคาะ
- ทำต่อ: **ฟอร์มออเดอร์ครบตามต้นแบบแล้ว รอเบสดูบน localhost ก่อน merge** — branch `nami/order-form-proto-full-20260917` ต้นแบบ `records/projects/anajak-erp/mockup-order-form-2026-09-18.html` ในสมอง · คงกฎจริงที่ต้นแบบไม่ได้วาด (ช่องกรอกกลาง · จำนวนสีเฉพาะสกรีน/รีดร้อน · ขนาดมาตรฐานล็อกกว้างยาว · การ์ดจอแคบ · ไม่มีเอฟเฟกต์ตอนชี้) · โมดูลผลิตไม่แตะ (หน้าใบผลิตยังใช้การ์ดชุดเก่า รอเคาะแยก)
