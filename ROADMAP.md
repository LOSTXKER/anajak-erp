...
- ทำถึงไหน: main มีโมดูลผลิตใหม่ (`ec0400b`) · branch `nami/orders-list-flow-20260916` หน้ารวมออเดอร์รอบโล่งขึ้น (รอเบสอนุมัติ) · **2026-09-17 เบสสั่ง "ทำให้ครบทุกหน้า เข้ากัน ใช้ component เดียวกัน refactor ได้"** → branch `nami/ui-unify-20260917` (ต่อจาก branch ออเดอร์) ขั้น U1 เสร็จ: ชิ้นกลางชุดเก่าหน้าตาแบบ kit — หัวหน้า (ชื่อใหญ่ ไม่มีกล่องไอคอนหมวด) · ช่องตัวเลข tile · ตาราง (ระยะแบบออเดอร์ ไม่มี hover) · ป้ายชิปพื้นอ่อน · ปุ่มมุม 12 ตอบสนองตอนกด · แถบแบ่งหน้ามีเลขหน้า (`lib/pager` ใช้ร่วมกับออเดอร์) · หัวการ์ด Section กล่องไอคอน · กล่องเตือนแบบ callout · หน้ารายการวางแถบค้นหาในการ์ดเดียวกับตาราง (`ResponsiveList toolbar`: ลูกค้า ใบเสนอราคา บิล ใบวางบิล หัก ณ ที่จ่าย ลูกหนี้) · ความกว้างหน้า 1124px ระยะ 18px เท่า kit
- ขั้นต่อ: U2 หน้ารายละเอียด/ฟอร์ม (ลูกค้า สินค้า ใบเสนอ เปิด/แก้ออเดอร์) · U3 ตั้งค่า 12 หน้า · U4 หน้าลิงก์ลูกค้า/login/จอโรงงาน · U5 รวมชิ้นซ้ำ kit กับ ui เหลือชุดเดียว (DueTag StatusDot Empty Callout Seg Tabs pager search) · U6 ลบของไม่มีผู้ใช้ (`lib/page-identity` หน้าผลิตเก่า /proto เก่า)
- ตรวจแล้ว: typecheck · lint 0 error · test 1866 ผ่าน · verify:ui ผ่าน (ปรับข้อกำหนดหัวหน้า/ปุ่ม/ตาราง/กล่องเตือนเป็นแบบ kit ไม่ได้ปิดด่าน) · เปิดหน้าเก่าด้วยตัวเรนเดอร์นอก Next (Tailwind+kit CSS จริง ข้อมูลตัวอย่าง): ลูกค้า บิล ใบเสนอ ใบวางบิล หัก ณ ที่จ่าย ลูกหนี้ ตั้งค่า ผู้ใช้ แจ้งเตือน ไม่พัง · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน**
- ติดอะไร: โมดูลผลิต ใบ V2 ติ๊กหน้าพิมพ์ DTF ไม่ได้ · แจ้งเตือนร้านนอกต้องมี `CRON_SECRET` · ออเดอร์ "คนทำ" ช่วงออกแบบเป็นผู้เปิดออเดอร์ · ชุดเก่ายังมี hover ในชิ้นที่ยังไม่ได้แตะ (เมนู/ชิปอื่น)
- ทำต่อ: เดิน U2–U6 บน `nami/ui-unify-20260917` → เบสเปิด preview ไล่ดูทุกหน้า → อนุมัติ merge ทั้งสอง branch ขึ้น main
