...
- ทำถึงไหน: main มีโมดูลผลิตใหม่ (`ec0400b`) · branch `nami/orders-list-flow-20260916` หน้ารวมออเดอร์รอบโล่งขึ้น · **2026-09-17 เบสสั่ง "ทำให้ครบทุกหน้า เข้ากัน ใช้ component เดียวกัน refactor ได้ · ทำให้เสร็จให้หมด"** → branch `nami/ui-unify-20260917` (ต่อจาก branch ออเดอร์ · ทั้งคู่ยังไม่ขึ้น main): ชิ้นกลาง `ui/` หน้าตาเท่า kit ทั้งชุด — หัวหน้า (ชื่อใหญ่ ไม่มีกล่องไอคอนหมวด) · tile ตัวเลข · ตาราง · ป้าย/InfoChip แคปซูลพื้นอ่อน · ปุ่มมุม 12 · แถบแบ่งหน้าเลขหน้า (`lib/pager` ร่วมกับออเดอร์) · หัวการ์ด Section/Card ระยะ 18px · กล่องเตือน callout · แท็บ · หน้ารายการวางแถบค้นหาในการ์ดตาราง (`ResponsiveList toolbar`) · ทุกหน้ากว้าง 1124px ระยะ 18px · ถอด hover ทั้งเว็บ 246 จุด + การ์ดกดได้ · ลบไฟล์ไม่มีผู้ใช้ 6 ไฟล์ (flow-filter-bar date-range-picker page-identity order-items-editor order-info-edit-dialog product-adaptive-card)
- ตรวจแล้ว: typecheck · lint 0 error · test 1865 ผ่าน (ถอดข้อทดสอบของไฟล์ที่ลบ 1 ข้อ) · verify:ui ผ่าน (ข้อกำหนดหัวหน้า/ปุ่ม/ตาราง/กล่องเตือน/hover เปลี่ยนเป็นแบบ kit ไม่ได้ปิดด่าน) · เรนเดอร์นอก Next ด้วย Tailwind+kit CSS จริงครบ 35 หน้า (หลังบ้าน ฟอร์ม ตั้งค่า login ลิงก์ลูกค้า จอโรงงาน) ไม่พัง · หน้าลูกค้าใส่ข้อมูลตัวอย่างตรงแบบหน้าออเดอร์ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน · หน้ารายละเอียดยังไม่ได้ใส่ข้อมูลตัวอย่างดูเต็ม**
- ติดอะไร: ชิ้นซ้ำ kit กับ `ui/` (DueTag StatusDot Empty Callout Seg Tabs) หน้าตาเท่ากันแล้วแต่ยังเป็นสองไฟล์ เพราะ kit ใช้ CSS module ใต้ `.tokens` ส่วน `ui/` ใช้ Tailwind — รวมเป็นไฟล์เดียวต้องย้ายตัวแปรสี kit ไป global ก่อน · โมดูลผลิต ใบ V2 ติ๊กหน้าพิมพ์ DTF ไม่ได้ · แจ้งเตือนร้านนอกต้องมี `CRON_SECRET` · ออเดอร์ "คนทำ" ช่วงออกแบบเป็นผู้เปิดออเดอร์ · `mockup-panel` ไม่มีผู้ใช้ในโค้ดแต่ SPEC ยังอ้าง (ไม่ลบ)
- ทำต่อ: เบสเปิด preview ของ `nami/ui-unify-20260917` ไล่ดูทุกเมนู → แก้ตามคำทัก → อนุมัติ merge ทั้งสอง branch ขึ้น main
