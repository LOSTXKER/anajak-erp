...
- ทำถึงไหน: 2026-09-15 ค่ำ รอบ "โล่งขึ้น" 2 ชุด บน branch `nami/orders-calm-20260915` ตามข้อที่เบสเลือกจากหน้าลอง (สมอง `records/projects/anajak-erp/mockup-orders-calm-2026-09-15.html`): รายการ = หัวตารางไม่มีพื้นเทา เส้นแถวจาง · หัวใบบรรทัดเดียว ไม่มีป้ายลูกค้าเห็น · รางขั้นเต็มแถว เส้นสีวิ่งเส้นเดียวถึงขั้นปัจจุบัน · กล่องตัวเลขไม่มีบรรทัดเล็ก/ไซส์ · ลูกค้า = กล่องเดียว (พรีวิว + ข้อมูลออกบิลพับ) ถอดประวัติซื้อไปหน้าลูกค้า · จัดส่งเป็นพรีวิว · รายละเอียดงานมีหัวข้อทุกช่อง · เอาเส้นเวลาออก · แท็บรายการไม่มีกรอบเทา/ยอดซ้ำหัวชุด · ไม่มีไอคอนซ้อนกันแนวตั้ง (งานผลิต ใบเสร็จ ใบส่งของ ของที่ต้องพร้อม)
- ตรวจแล้ว: typecheck · lint 0 error · test 188 ไฟล์ · verify:ui (ปรับด่านหัวใบ/ราง/ภาพรวม/ลูกค้าตามข้อใหม่) · เรนเดอร์ชิ้นจริงเป็น HTML นิ่ง: วัดเส้นสีเริ่ม/หยุดกลางวงตรง · กล่องลูกค้า · ไอคอนไม่ซ้อน · ชุดงานไม่มีกรอบ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน** (Chrome ของเบสต่อไม่ติด)
- ติดอะไร: แท็บรายการของจริงดูผ่าน CSS + โครงจำลอง ไม่ได้เรนเดอร์คอมโพเนนต์ตรง (ต้องใช้ข้อมูลจากฐาน) · ปุ่มโทร/แชทในภาพรวมหาย (ทำที่หน้าลูกค้า) · รายละเอียดงานของจริงมีแค่ชื่องาน/แบรนด์/สี/ฟอนต์/หมายเหตุสไตล์ · ข้อ 6 (ป้ายสถานะซ้ำข้างเลข) และ 8 (หมายเหตุกล่องขาว) เบสยังไม่ตอบ จึงไม่ทำ · ค้างจากรอบก่อน: ไม่ลองอัป/ลบไฟล์ · ปุ่มตรวจนับ QC ใบ V2 และ "วางบิลแล้ว" นับซ้ำ แยกงานแล้ว
- ทำต่อ: เบสเปิด `localhost:3000/orders` บน branch นี้ดูของจริงทุกแท็บ → แก้ตามคำทัก → เบสอนุมัติแล้ว merge ขึ้น main (ห้าม push main เอง) · ตอบข้อ 6/8
