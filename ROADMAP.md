...
### A4. แบ่งตามที่ยืน — ใบผลิต = ดู+วางแผน · จอสถานี = ลงมือ (เบสเคาะ "เอา A" 2026-09-03)
- โจทย์: เบสถาม (09-03) "ใบผลิต `/production/[id]` กับจอสถานี `/station` ซ้ำซ้อนกันไหม หัวหน้าจะงงไหมว่าทำอันไหน" → ตรวจแล้วซ้ำจริงครึ่งเดียว: **ปุ่มลงมือชุดเดียวกันอยู่ทั้ง 2 หน้า น้ำหนักเท่ากัน ไม่มีกติกาว่าทำที่ไหน** + จอสถานีไม่อยู่ในเมนู + ช่างเปิดหน้าการผลิตทั้งหน้าได้ · เสนอ 3 ทาง (A แบ่งตามที่ยืน / B ป้ายบอกทางอย่างเดียว / C ยุบสถานีเป็นตัวกรอง) → **เบสเคาะ A**
- โครง (ลง `SPEC.md` §UI เมื่อเคาะหน้าตา): **จอสถานี = ยืนหน้างาน → ลงมือทุกอย่างที่นี่ที่เดียว** (เริ่ม · ลงยอด · ปิดขั้น · เบิกเสื้อ · ตรวจรับของกลับ · แจ้งปัญหา · หัวหน้าแก้ให้) · **ใบผลิต/หน้าการผลิต = นั่งโต๊ะ → ดูทั้งใบ + วางแผน** (มอบหมาย · ควรเสร็จ · ส่งร้านนอก/นัดรับ/เปลี่ยนร้าน · เสื้อ-วัตถุดิบ · ต้นทุน) **ไม่มีปุ่มลงมือ** — ทุกขั้นบอกว่าอยู่สถานีไหน ใครถือ + ปุ่ม "ไปทำที่จอสถานี" พาไปงานนั้นตรง ๆ
- [x] หน้าลอง `/proto/desk-station` (09-03 · branch `proto/desk-station-2026-09-03`): ปัจจุบัน (D มีปุ่มลงมือ) / **A คงโครง D แทนโซนลงมือด้วย "การ์ดที่ยืน"** / **B ตารางแผนทั้งใบ** · สวิตช์หัวหน้า/ช่าง · ธีม · ตารางโครง "ใคร-ยืนไหน-หน้าไหน-ทำอะไรได้" บนหน้า — **รอเบสเคาะ A/B**
- [ ] ลงของจริงหลังเคาะ (ถอดเป็นหลัก ไม่สร้างใหม่ ~1 วัน): ใบผลิต `work-order-page` ถอดโซนลงมือ/ปุ่มแจ้งปัญหา/ปุ่มสถานะหัวหน้า → การ์ดที่ยืน + ลิงก์ `/station?st=&s=job&job=&step=` · คง dialog โหมดหัวหน้า (มอบหมาย/แผน) + ร้านนอก (ส่ง/นัดรับ) + วัตถุดิบ · เมนูซ้ายเพิ่ม "จอสถานี" (`lib/navigation.ts`) · ช่างที่มีแค่ `manage_production` ล็อกอินแล้วตกที่ `/station` (ดู redirect หลังล็อกอิน — ถามก่อนถ้าต้องแตะ auth) · ปุ่ม "จอสถานี" บนหน้าการผลิตยังอยู่ · `docs/DESIGN.md` route table + §Station ปรับตาม · `station-job` ไม่ต้องแก้ (ใช้ controller ชุดเดิม)
- [ ] เก็บตกที่ยังไม่ตัดสิน: ขั้นที่ทำที่โต๊ะโดยธรรมชาติ (เบิกเสื้อผ่านสต๊อกจากคอมของหัวหน้า?) — ถ้าทีมทำจริงที่โต๊ะ ให้เปิดจอสถานีบนคอมได้เหมือนกัน ไม่ต้องคืนปุ่มให้ใบผลิต
