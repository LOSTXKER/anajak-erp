...
- ทำถึงไหน: 2026-09-15 เก็บหน้าแรกและออเดอร์ตามสองลิงก์ที่เบสส่ง บน `codex/ui-reference-polish-20260915`: หน้าแรกกว้าง 1124px หัว 28px การ์ด/ผัง/ตัวกรอง/ตารางแยกต้องจัดการกับขั้นงาน/เงินตัวเลข mono; เมนู 244px และหัวเว็บเข้าชุดกัน; ออเดอร์เก็บระยะมือถือ/ไอคอนและรูปดูย่อไม่ล้นหรือทับชื่อ. main ยังเป็นงานรอบก่อน (`5603dfa`); รอบนี้ยังไม่ปล่อย Production
- ตรวจแล้ว: เปิดต้นแบบทั้งสองใน Chrome เทียบกับ `dev:demo` จริง; OWNER ที่ 1440px/390px สว่างและมืด: หน้าแรกเลือกวัน→เหลือ 3 ออเดอร์, กรองว่าง→ล้าง, เมนูหุบ/กาง, ออเดอร์ค้นว่าง→ล้าง/ดูย่อ/Esc/เปิดใบ/แท็บภาพรวม รายการ เงิน. มือถือ main กว้าง 380px และ scrollWidth 380px; ดูย่อ 1440×720 รูปอยู่ในกรอบและชื่ออยู่ใต้ภาพ. typecheck · lint 0 errors/22 warnings · 188 ไฟล์/1,859 tests · verify:ui ผ่าน; ผู้ตรวจอีกคนอ่าน diff ไม่พบ regression
- ติดอะไร: รอบนี้ตรวจบัญชี OWNER; ยังไม่ลองบทบาทช่าง/บัญชีและอัป/ลบไฟล์ (dev:demo อัปขึ้น Supabase จริง). ข้อมูลตัวอย่าง/จำนวนจึงต่างจากต้นแบบ; เครื่อง DTF แสดงจำนวนรอบพิมพ์จริง ไม่อ้างว่าเครื่องปกติ. ปัญหาเดิม QC ใบ V2 ถูก server ปฏิเสธ และ "วางบิลแล้ว" นับใบเสร็จซ้ำยังแยกงาน. คงทางมือถือแบบแถบล่างและรางเมนูหุบ 64px ของระบบเดิม
- ทำต่อ: เบสเปิดหน้าแรกและ `/orders` บน localhost เทียบตัวอย่าง; เมื่อรับหน้าตาแล้วจึง merge/release ตามอนุมัติ. ลองหลายบทบาทและอัปไฟล์บนที่เก็บทดลองต่อใน A16

### ขอบเขตและจุดส่งต่อ A16

รอบ 2026-09-15 เบสสั่งเก็บ UI ให้ตรงตัวอย่างหน้าแรก `P4C3jsTZ2p98Pq6ubxR9wK` และหน้าออเดอร์ `2e0c6550-5966-4181-b0b9-02e2715939ed`: เทียบจอจริง → เก็บเมนู/หัวเว็บ/หน้าแรก/รายการและรายละเอียดออเดอร์ → ลอง desktop/mobile สองธีมและด่านเดิม; คงข้อมูล บริการ สิทธิ์ และสถานะ ทำบน branch ไม่ปล่อย Production ในรอบนี้
