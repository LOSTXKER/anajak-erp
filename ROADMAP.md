...
- ทำถึงไหน: 2026-09-14 เบสสั่งรื้อ UX/UI แนว minimal modern (Notion/Vercel) มุมโค้ง มีลูกเล่น คงน้ำเงิน Anajak และฟอนต์ Prompt; ทำหน้าแรกเป็นต้นแบบก่อน. ต้นแบบ = HTML เดี่ยวในสมอง `records/projects/anajak-erp/mockup-home-minimal-2026-09-14.html` (artifact `b29b1705`) มีสวิตช์เทียบแบบเดิม 4 มุมมอง; โค้ดแอปยังเท่า `7691bf7`. เบสเคาะทิศแล้ว; การ์ดซ้ายบนผ่านมา 4 รอบ (ตัวเลขนับ → รายการงาน → ผัง/สุขภาพแยก 2 แบบ → รอบ 4 รวมการ์ดเดียว): แถวสุขภาพ 4 ช่อง + ผังรางตรง 2 สาย (สายเสื้อ ออกแบบ→เตรียมเสื้อ→ร้านนอก · สายฟิล์ม พิมพ์ฟิล์ม DTF) บรรจบก่อนรีดร้อน → QC → แพ็ก → ส่ง; node จริงจาก `factory-station.ts`/`production-steps.ts` มีจุดสถานะ แถบงานในมือ/กำลัง คอขวด; รายการต้องจัดการเป็นคอลัมน์ในตารางออเดอร์
- ตรวจแล้ว: ต้นแบบรอบ 4 เปิดจริงบน 1440 และ 375 ทั้งสว่าง/มืด ไม่ล้นแนวนอน (ผังเลื่อนในการ์ดเองบนมือถือ); รางวาดตามตำแหน่งจริงและวาดใหม่เมื่อย่อจอ มุมมองไม่เห็นเงิน/โรงงานปกติ/กำลังโหลดทำงาน. dev server พอร์ต 3000 ดับระหว่างตรวจ จึงตรวจผ่านตัวเสิร์ฟไฟล์นิ่งชั่วคราว (ปิดแล้ว). รอบนี้ไม่แตะโค้ดแอป จึงไม่มีผล typecheck/test ใหม่
- ติดอะไร: รอเบสเคาะการ์ดโรงงานรอบ 4; ต้องมี query ใหม่ก่อนลงจริง (นับใบงานต่อสถานีจาก `factory.board`, ด่านรีดร้อนรอฟิล์ม/เสื้อจาก `evaluateHeatPressGate`, งานร้านนอกเลยรับ, กำลังต่อวันจาก WorkCenter ซึ่งยังว่างจนกว่า V2 cutover, ส่งตรงเวลา 7 วันตาม MFG1); หน้าลอง `/proto` ในโค้ดถูกลบตั้งแต่ 09-11
