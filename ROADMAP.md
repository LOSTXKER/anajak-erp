...
- ทำถึงไหน: main = `94a1949` (ขึ้นเว็บจริงแล้ว 2026-09-16) · **2026-09-16 เบสสั่ง "ทำแบบ proto นี้เลยแบบ 100% ทุกหน้าทุกส่วน ยกเว้นออเดอร์และการผลิต" พร้อมสั่งว่าฟีเจอร์จริงที่ต้นแบบทำหายให้เก็บไว้** → แตกโค้ดต้นแบบเป็นไฟล์รายหน้า 44 หน้า แล้วเทียบกับโค้ดจริงทีละช่อง พบส่วนต่าง 353 จุด (หนัก 58 กลาง 153 เบา 142) และไล่แก้ทั้งชุดบน `nami/ui-pages-20260916`
...
- ตรวจแล้ว: typecheck · lint 0 error (เตือน 17 จาก 27) · test 1865 ผ่าน · verify:ui ผ่าน 64 ตก 0 · เรนเดอร์นอก Next ดูจริง 7 หน้า (ลูกค้า · งานของฉัน · ตั้งค่า · บิล · รายงาน · เข้าสู่ระบบ · ลิงก์สถานะงาน) ตรงต้นแบบ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน**
- ติดอะไร: ต้นแบบขัดข้อบังคับบางข้อจึงคงของจริงไว้ — เอกสารพิมพ์ (ต้นฉบับ/สำเนา · ลายน้ำยกเลิก · จำนวนเงินเป็นตัวอักษร · ส่วนลด · ใบลดหนี้/เพิ่มหนี้ · ใบแพ็คห้ามมีเงิน · ใบสั่งงานต้องมี QR+เวอร์ชันม็อกอัพ) และลิงก์ลูกค้า (ขอบเขตข้อมูลต่อ token · blind ship ปิดตัวตนร้าน) · ด่าน verify:ui ปรับ 3 ข้อให้ตรงต้นแบบใหม่ (ความกว้างราง · สถานะหุบ · รูปแบบ map กลุ่มเมนู) ไม่ได้ลดความเข้ม · ที่ยังไม่ทำเพราะต้องแตะ API/ด่านคู่กัน: ตัวเลขงานค้างของเมนู "งานของฉัน" · กล่องลอยแจ้งเตือนบนกระดิ่ง · หัวคอลัมน์ตารางพื้นเทา · โหมดมืดสลับให้รางสว่างกว่าพื้น
- ทำต่อ: **ออเดอร์และการผลิต — เบสสั่งทำเป็นต้นแบบก่อน (16 ก.ย.)** ต้นแบบกดเล่นได้ที่ `records/projects/anajak-erp/mockup-orders-production-2026-09-16.html` ในสมอง: ออเดอร์ 5 หน้า การผลิต 7 หน้า และกล่องโต้ตอบ 16 กล่อง · **17 ก.ย. เบสเคาะกติกา "เอาโครงของจริง เปลี่ยนแค่หน้าตาให้เข้าธีมใหม่"** ทำแล้วทั้ง ฟอร์มเปิดงาน/แก้ไข/แก้ไขรายการ (4 แท็บ ฟอร์มเดียว) · โหมดหน้างาน (เลิกเป็นจอดำ ตามที่ตีกลับไว้ 09-02) · จอ TV · และแท็บรายการที่รื้อเป็นตารางไม่ซ้ำข้อมูล · รอเบสไล่ดู · โค้ดจริงของสองโมดูลยังไม่แตะ (รวมเส้นแดงในหน้าออเดอร์ที่เคยขอคืน)
