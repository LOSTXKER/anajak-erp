...
- ทำถึงไหน: 2026-09-15 หน้าออเดอร์ขึ้น main `eb87269` ตามเบสสั่ง (Vercel + CI check ผ่าน) · เบสบอก "ไม่เหมือน…รื้อเขียนใหม่" → ยก CSS ต้นแบบตรงตัวที่ `orders.module.css` + `c()` ใน `orders-ui.tsx` · เขียนใหม่รายการ/ดูย่อ/หัวใบ/ราง/7 แท็บ (`detail/order-*-tab.tsx`) · ตรรกะเงิน/สิทธิ์/สถานะ/V2 คงเดิม
- ตรวจแล้ว: typecheck · lint 0 error · test 188 ไฟล์ · verify:ui (แก้สัญญาคลาสเก่า + stub `.css`) · วัดกล่องเทียบต้นแบบต่างไม่กี่ px ทุกแท็บ · มืด · 375px · ธุรกรรม demo: สภาพเสื้อ, รับเงิน→ออกใบเสร็จ, เลขพัสดุ→ส่งแล้ว, รับเสื้อลูกค้า, เมนู ⋯, ดูย่อ ↑↓/Esc ผ่านตรงฐาน
- ติดอะไร: ไม่ลองอัป/ลบไฟล์ (dev:demo อัปขึ้น Supabase จริง) · ปุ่มตรวจนับ QC ใบ V2 โผล่แต่ server ปฏิเสธ และ "วางบิลแล้ว" นับใบเสร็จซ้ำ — ของเดิมทั้งคู่ แยกงานแล้ว · ต่างต้นแบบโดยตั้งใจ: ไม่มีใบปะหน้า/ส่งลิงก์บิล/ปุ่มเริ่มผลิต · ของเดิมไม่ใช้แล้วแต่ยังเก็บ: `order-delivery-section.tsx` `order-files-card.tsx` `MockupPanel`
- ทำต่อ: เบสเปิด `localhost:3000/orders` เทียบต้นแบบ → แก้ตามคำทัก → ลองบทบาทอื่น (ช่าง/บัญชี) และอัปไฟล์บนที่เก็บทดลอง · งานแก้ต่อทำบน branch แล้วขึ้น main ตามอนุมัติ (ห้าม push main เอง)
