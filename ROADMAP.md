...
- ทำถึงไหน: main = ทุกหน้าตามต้นแบบ ยกเว้นออเดอร์/ผลิตที่คงฟีเจอร์จริง (เบสสั่ง 2026-09-16 ให้ตรง 100%) · ฟอร์มออเดอร์ครบตามต้นแบบ · ชุดกลาง kit/PageShell/`rowToneFor` ใช้ร่วมทุกหน้า
- ตรวจแล้ว: typecheck · lint 0 error (เตือน 17 เท่าเดิม) · test 1941 ผ่าน · verify:ui ผ่านครบ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน**
...
- [ ] **งานแก้และเคลมหลังลูกค้ารับของ** (เบสสั่ง 2026-09-18 · อนุญาตให้รื้อถึงราก) — ก้อน 0 ขึ้นเว็บแล้ว (`7a8d5aa` · เกณฑ์อยู่ใน SPEC) · **กำลังทำก้อน 1 = ใบเคลม**: ตาราง `OrderClaim` + `OrderClaimLine` (รายไซซ์ — เบสยืนยันว่าหน้างานนับ "S เสีย 3 M เสีย 2") · เปิดอัตโนมัติเฉพาะตอนตีกลับใบส่ง · ปุ่มเดียวสั่งงานแก้ (ถอยทีละขั้นผ่าน transitionOrder + เปิดขั้นพร้อมจำนวน) · ปิดใบไม่ได้จนกว่าเอกสารเงินจะออกจริงและยอดตรง · สิทธิ์ใหม่ `decide_claims` (เจ้าของ/ผู้จัดการ/ฝ่ายขาย) เอกสารเงินยังใช้ `manage_billing_docs` เดิม · ไม่เพิ่มแท็บ/การ์ด: แถบบนใบออเดอร์ + เหตุ `claim` ใน home-orders (โผล่ครบ 4 จอ) + หน้าลูกค้าพูดว่ากำลังแก้งาน · **ตัดออกจากก้อน 1**: ตีกลับบางส่วน (ก้อน 2) · แก้รายการใต้ใบเคลม (ก้อน 3) · สั่งงานแก้บนใบผลิต V2 (วันนี้ทำไม่ได้ ต้องขึ้นเหตุผลบนจอ) · ปิดรูที่เจอ: เหตุผลถอยสถานะรั่วยอดเงินให้ role ที่ไม่เห็นเงิน (`revision-policy.ts` ยกเว้นชนิด STATUS) · คู่มือ 80 เคส: https://claude.ai/artifact/MtyRHyJgxr9KGxtvQhTHzY · รอเบสเคาะ: รับเคลมกี่วัน · ริบมัดจำ · คืนเงินหรือเครดิต · ค่าของที่ทำทิ้ง
