...
- [ ] **R1 canonical UI architecture** — ย้าย shell/dashboard/order helpers ตัวจริงออกจากชื่อ `v2-*` · update import/test · ลบ Sidebar/Topbar/MobileSidebar เก่าที่ `rg` ยืนยันว่าไม่มี caller · คง `/v2*` compatibility redirect ไว้
- [ ] **R2 แท็บเบาและสิทธิ์นิ่ง** — `TabsContent` lazy เป็น default · ฟอร์มเปิดงาน opt-in `keepMounted` · order detail ไม่ยิง query ของ 6 แท็บพร้อมกัน · รอ permission ก่อนวาด action และบอกเหตุผลเมื่อ deep-link ไปแท็บที่ไม่มีสิทธิ์
- [ ] **R3 interaction truth + accessibility** — DatePicker/DateRange ไม่มี button ซ้อน · clear/close target ≥44px บนมือถือ · เปลี่ยน delivery placeholder ที่ handler ว่างเป็น read-only state · แก้ป้ายทางลัด dashboard ไม่ให้ตัดคำจนเดาไม่ได้
- [ ] **R4 multi-user freshness** — `/production` poll 30 วินาที + refetch เมื่อกลับแท็บ โดยคง optimistic/mutation invalidation เดิม · ไม่เพิ่ม realtime infrastructure
- [ ] **R5 draft ครบใบ** — ใช้ header state ก้อนเดียวที่มีอยู่แล้ว · กู้ channel/deadline/notes/priority/payment terms/PO/discount/platform fee/tax รวมถึง fees/shipping และ metadata ไฟล์ที่อัปโหลดแล้ว · ไม่เก็บ base64 preview ลง localStorage
- [ ] **R6 verify เหมือนงานจริง** — desktop 1280 + mobile 390 บน dashboard/orders/new/detail/production/print-runs · วัด overflow/target/query/error/console · keyboard/Escape/focus return · typecheck/lint/unit/verify:ui/Impeccable detector/build ผ่าน · อัปเดต DESIGN/PROGRESS
