...
- ทำถึงไหน: 2026-09-14 เบสเคาะทิศ minimal modern (ต้นแบบ HTML ในสมอง `records/projects/anajak-erp/mockup-home-minimal-2026-09-14.html`) แล้วสั่ง "ทำของจริงเลย" → ลงหน้าแรกจริงบน `nami/home-mockup-20260914`: service `home-overview.ts` + `analytics.homeOverview` (node โรงงานจาก `factory-station.ts` · ด่านรีดร้อนรอฟิล์ม/เสื้อจาก `evaluateHeatPressGate` · กำหนดส่ง 7 วัน · ออเดอร์ที่กำลังเดินพร้อมเหตุที่ต้องจัดการ · เงิน gate `see_finance`) · UI `src/components/dashboard/home/` (ตัวเลข 4 ช่อง → ผังราง 2 สาย+สุขภาพ → ตารางออเดอร์เรียงเรื่องด่วน | กำหนดส่ง 7 วัน + เงินที่ต้องตาม) · token ทั้งเว็บ: ผืนงานขาว การ์ดขอบบางไม่มีเงา มุม 20px เมนูซ้ายเทาอ่อน เมนูที่เปิดอยู่ฟ้าอ่อน · ด่าน verify:ui และ brand-selection test ปรับตามทิศใหม่
