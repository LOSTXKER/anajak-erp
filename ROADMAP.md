...
### รอทบทวนต้นแบบหลังลดความรก

- คำขอล่าสุด 13 ก.ย. 2569: ลดความรกในผู้ติดต่อ/งานผลิต/จัดส่ง/ประวัติแล้ว. ช่องทางติดต่อเป็นแถวสั้นพร้อมคำสั่งคัดลอกที่มองเห็นได้; เลขภาษีและกลุ่มลูกค้าอยู่ในปุ่มข้อมูลลูกค้า คงหมายเหตุติดต่อที่จำเป็น. ลดกรอบปุ่มและป้ายซ้ำ ให้ชื่อเหตุการณ์เป็นลิงก์เส้นใต้พร้อมลูกศรแทนปุ่มเปิดแยก; คง Prompt พื้นขาวและสีเน้นเดิม
...
- แท็บที่เหลือ: `?tab=production` แสดงสินค้า/ไซซ์/งานพิมพ์คู่กับสถานะม็อกอัพและไฟล์เป็นแถวลิงก์; ใบสั่งงานเปิดจากหัวออเดอร์จุดเดิม. `?tab=delivery` แสดงผู้รับ/ที่อยู่คู่กับรายการแพ็กและตัวอย่างใบปะหน้า มีสถานะรอบจัดส่งบรรทัดเดียว. ยอดจำนวนรวมแสดงครั้งเดียวในแต่ละแท็บ. `?tab=history` เรียงเหตุการณ์ตามวัน กรองข้อมูล/ไฟล์/เงิน เปิดส่วนที่เกี่ยวข้องจากชื่อเหตุการณ์ และเห็นค่าก่อน–หลังเมื่อแก้ข้อมูล. จำนวนและเหตุการณ์มาจากข้อมูลตัวอย่างและการกดในหน้าลอง ไม่สมมติว่างานผลิตเริ่มหรือจัดส่งแล้ว
...
- ตรวจรอบนี้: ดูทั้งสี่ส่วนบน 1440/390 และ Light/Dark, Prompt เดิม รูปโหลดครบ หน้าจอไม่ล้น; คัดลอกแล้วมีผลสำเร็จ เปิดข้อมูลลูกค้าได้พร้อมเลขภาษี/กลุ่มที่ย้าย; เปิดไฟล์พิมพ์ถูกกลุ่ม; แก้ที่อยู่/ยกเลิกและเปิดใบปะหน้าได้. เปิดเหตุการณ์ด้วย Enter ไปแท็บถูกต้อง ตัวกรองคงเมื่อกลับมา และกรณีหมวดไฟล์ว่างมีปุ่มดูทั้งหมด. ลองงานผลิตที่ไม่มีม็อกอัพ/ไฟล์แล้ว; ไม่พบ console error/warning. ตรวจ React และคำบนจอตาม ui-guidance แล้ว คืนหน้าลองเป็น Light และขนาดปกติ. ภาพใหม่อยู่ `.tmp-test/order-workspace-quiet-20260913/`
- ด่านตรวจรอบนี้: typecheck ผ่าน; lint 0 errors/22 warnings เดิม; 201 files/1,971 tests ผ่าน; verify:ui 255 + work-order 32 ผ่าน. หลังเก็บข้อความผลคัดลอกและกรอบโฟกัส ตรวจ lint ไฟล์ที่แก้ซ้ำผ่าน. Log `quiet-tabs-*` อยู่ `.tmp-test/ui-polish-20260913/` ไม่เข้า Git. สคริปต์ proto-shots เดิมไม่พบทั้งต้นฉบับและสำเนาติดตั้ง จึงตรวจภาพผ่าน browser โดยไม่ได้อ้างผลผ่านสคริปต์เก่า
