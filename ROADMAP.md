...
- ทำถึงไหน: 2026-09-16 ลงโมดูลผลิตตามต้นแบบ `mockup-production-calm-2026-09-15` (บัตรสมอง) บน branch `nami/production-module-20260916` (ต่อจาก `nami/remove-left-stripes-20260916` ถอดแถบสีขอบซ้ายทั้งเว็บ · ยังไม่ขึ้น main): หัวโมดูล+เมนูย่อยร่วม (`production-module-head`) · `/production` กล่องสถานะ 4 ช่อง + ตารางแยกคอลัมน์ร้านนอก (`production-desk-kit`) · `/production/[id]` แจ้งเตือนบนสุด เส้นงานเรียบ การ์ดขั้นมีรูปลายคู่ตำแหน่ง/ค่ารีด ยอดต่อไซซ์ตรงแนว เช็คลิสต์/ข้อมูลออเดอร์/ประวัติขั้นงาน แท็บสินค้าใหม่ (`work-order-kit` · ปุ่มจาก controller เดิมผ่าน `options.kit`) · `/production/print-runs` พิมพ์ DTF ติ๊กหลายใบกดพิมพ์เสร็จครั้งเดียว (เรียก create→markPrinted→complete เดิม · รอบค้างมีปุ่มปิดให้เสร็จ) · `/production/outsource` ใบส่งร้าน ปุ่มจาก `availableCommands` + กู้ `outsource-share-dialog` · `/production/metrics` MFG1–3 (`services/production-metrics` + `analytics.productionMetrics`) · cron overdue แจ้งหัวหน้าเมื่อร้านนอกเลยนัดรับครั้งเดียวต่อใบ · คลังฟิล์มไม่มีในเมนู (เบสตอบ: พิมพ์เผื่อไม่ค่อยมี)
- ตรวจแล้ว: typecheck · lint 0 error · test ครบ (เพิ่ม production-metrics 5 ข้อ · outsource-overdue 1 ข้อ) · verify:ui ผ่าน · เรนเดอร์ทุกหน้าด้วยข้อมูลตัวอย่าง/fixture หน้าลองเดิมที่ 1180px: ตารางไม่ล้นการ์ด ตัวเลขยอดตรงแนว (วัดขอบขวา) ใบผลิต 4 สถานะ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอินและยังไม่ลองกดบันทึกบนฐานทดลอง**
- ติดอะไร: ใบ V2 ติ๊กในหน้าพิมพ์ DTF ไม่ได้ (ต้องรายงานราย quantity line) · หน้าเก่า `work-order-page`/`production-desk-view`/`-cells`/`-preview` ยังอยู่เพราะหน้าลอง `/proto` ใช้ · แจ้งเตือนร้านนอกทำงานเมื่อ cron มี `CRON_SECRET` · สูตร ⓘ ตัวชี้วัดเป็น tooltip (จอทัชไม่เห็น)
- ทำต่อ: เบสเปิดหน้าจริงบน localhost/preview → ลองบนฐานทดลอง: ติ๊ก+ยอด+ปิดขั้น · พิมพ์เสร็จ DTF หลายใบ · ส่งร้าน/รับกลับ/ตรวจรับ · เปิดตัวชี้วัด → แก้ตามคำทัก → เบสอนุมัติ merge ขึ้น main
