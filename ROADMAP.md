...
- ทำถึงไหน: main = ทุกหน้าตามต้นแบบ ยกเว้นออเดอร์/ผลิตที่คงฟีเจอร์จริง (เบสสั่ง 2026-09-16 ให้ตรง 100% → เทียบ 44 หน้า แก้ 353 จุด ขึ้นเว็บแล้ว) · ฟอร์มออเดอร์ครบตามต้นแบบ · ชุดกลาง kit/PageShell/`rowToneFor` ใช้ร่วมทุกหน้า
- ตรวจแล้ว: typecheck · lint 0 error (เตือน 17 เท่าเดิม) · test 1921 ผ่าน · verify:ui ผ่านครบ · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน**
...
- **ป้ายและถ้อยคำร่วมกันทั้งเว็บ — รอบ 1-5 จบ รอ merge** (เบสสั่งทำ 2026-09-18 หลังผลตรวจ 6 ด้าน 64 เรื่องที่ซ้ำจริง) — วิธีพิมพ์ · ประเภทลูกค้า · สถานะผลิต/ชำระ · เงิน · วันที่+เขตเวลาไทย (รวมวันที่ในไฟล์ CSV · ตัวเลข "เดือนนี้" ขยับขึ้นโดยตั้งใจ) · ชื่อผู้ติดต่อของนิติบุคคลไม่บังคับกรอกแล้ว · `cc1a118` `3f26446` `67acfb3` ผ่านด่านครบ **ยังไม่ได้เปิดหน้าจริงที่ล็อกอิน** · ค้าง: ชื่อบริการพิมพ์ในฐานที่ใช้จริงยังเป็นอังกฤษ (seed มีผลกับฐานที่สร้างใหม่เท่านั้น) ต้องแก้ที่ ตั้งค่า → บริการ · ผลตรวจและ mockup: สมอง `records/projects/anajak-erp/audit-shared-ui-2026-09-18.md` · https://claude.ai/artifact/CP6vSsuPYA7aFGUdepq2hr
- **รอเบสสั่ง merge `nami/priority-days-20260918`** — เปิดทางถอยจาก "พร้อมส่ง": กติกาถอยสถานะที่เบสเคาะ 2026-09-18 เขียนลง SPEC แล้ว · ลงจุดที่ขาด `READY_TO_SHIP → PACKING` ทางเดียว หัวหน้า+เหตุผล ปฏิเสธเมื่อมีใบส่งออกแล้ว (ตรวจหลัง transition ด้วย from จริง) · ปุ่มย้อนบนหัวใบออเดอร์/ใบผลิต · test ใหม่ 9 ข้อ (`a739fb1`)
- ยังค้างจากของที่ขึ้นเว็บแล้ว: ปุ่มย้อนหน้าผลิตยังไม่รู้จัก V2 (V2 ยังไม่เปิดจึงไม่กระทบ) · เลขรายการในกล่อง error ไม่ขยับตาม · ลบชุดงานยังไม่มีถามยืนยัน · `work-order-page.tsx` เป็นเชลล์หน้าลองที่ไม่ได้ใช้จริง รอเคาะว่าจะลบ
