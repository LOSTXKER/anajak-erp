...
**ก้อน 1 — กระดูกสันหลัง: ด่านพร้อมผลิต + เตรียมของ + เชื่อมสต๊อคจริง (เริ่มก่อน):**
- [ ] **ฝั่ง Stock** (repo `../anajaktshirt-stock` — ต้อง /add-dir): แก้ `POST /api/erp/movements` ให้ variant-aware (lookup variant SKU ควบ product SKU) + ห่อ `$transaction` + กันติดลบ + รับ `RETURN` + เซ็ต `orderRef` ราย line + idempotency key — refactor ดึง logic จาก `postMovement` (src/actions/movements/workflow.ts ที่ทำถูกแล้ว) เป็น service กลาง · ระวังบั๊กเก่า NULL variantId ใน unique index (มีสคริปต์ fix-stock-variants.ts เป็นหลักฐาน)
- [ ] **ฝั่ง Stock**: ตาราง `StockReservation` (variant/qty/qtyConsumed/orderRef/status) + endpoint จอง-ปลดจอง + `GET /stock` คืน qtyReserved/qtyAvailable + โชว์ยอดจองใน UI หน้า /stock (ฝ่ายผลิตใช้ทุกวัน) · API products คืน `variant.lastCost` ด้วย (ทุนจริงล่าสุด)
- [ ] **ฝั่ง ERP**: ยืนยันออเดอร์ FROM_STOCK → จองรายไซส์-สีอัตโนมัติ · ใบเบิก (ISSUE+orderRef ตัดยอดจอง) ผูกขั้น GARMENT_PICK · ใบคืนเศษ (RETURN) · เสื้อโรงเย็บ→PO/GRN ฝั่ง Stock (โรงเย็บ=supplier)
- [ ] **ฝั่ง ERP**: ด่านพร้อมผลิต (เงินตามเทอม ✓ + แบบอนุมัติ ✓ + ของครบ ✓) — รายการเตรียมของ auto จากเนื้อออเดอร์ · งานติดด่านห้ามโผล่คิวช่าง (แยกกอง "ติดอะไร รอใคร")
- [ ] **ฝั่ง ERP**: ใบตรวจรับของเข้า (นับจริงต่อไซส์ + รูป + ตำหนิ — mobile-first) สำหรับเสื้อลูกค้า/เสื้อโรงเย็บ/รับกลับร้านนอก + ใบคืนของลูกค้า (กระทบยอดรับ-คืน) · ขาด/เกินเด้งแจ้งแอดมินทันที
- [ ] **ฝั่ง ERP**: ProductionStep มี qty (done/total — บอก "บางส่วน" ได้) + ใบส่งร้านนอกแบ่งส่งหลายรอบได้ (ปลดล็อกงานค้างทีละใบ)

**ก้อน 2 — หน้าเครื่อง:** รอบพิมพ์ฟิล์ม (รวมหลายออเดอร์ กดเสร็จเป็นชุด) + จุดตัดแยกฟิล์ม + คลังฟิล์มพร้อมรีด (พิมพ์เผื่อ ติดป้ายลาย/ลูกค้า) + คิวรีด gate ฟิล์ม∧เสื้อ + จอเช้า 5 บทบาท + 5 ตัวเลขเจ้าของ + เรตต้นทุนกลาง (Settings ~5 ช่อง → กำไรขั้นต้นอัตโนมัติตอนตีราคา + เตือนเมื่อทุนซื้อเบี่ยง >10%)
**ก้อน 3 — ขาออก:** QC เชิงนับ (เสีย×ไซส์×ลาย×สาเหตุ×รูป) + เผื่อเสีย default 3% + แพ็คนับยืนยัน + รายการต่อกล่อง + blind ship + แบ่งส่ง + WHT ขารับ 3% (ของเดิมใน P1)
**ก้อน 4 — โฉมใหม่:** UI ทั้งระบบ (ทำภาพให้เบสเลือกก่อน) + ทางลัดงานชิ้นเดียว + ใบแก้ไขออเดอร์ (change order)
