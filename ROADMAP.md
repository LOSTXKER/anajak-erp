...
- ทำถึงไหน: 2026-09-16 โมดูลผลิตใหม่ + ถอดแถบสีขอบซ้ายขึ้น main แล้ว (`ec0400b`) · ต่อด้วยหน้ารวมออเดอร์รอบโล่งขึ้นตามต้นแบบ `mockup-orders-list-lite-2026-09-16` (บัตรสมอง) บน branch `nami/orders-list-flow-20260916`: หัวไม่มีบรรทัดสรุป · เส้นสถานะไม่มีกรอบ (`order-pipeline` grid ล้วน) · ตัวกรองวันที่เป็นปฏิทิน+ช่วงสำเร็จรูป (`kit/date-range`) · ป้ายสถานะที่เลือกล้างได้ · ตาราง 9 คอลัมน์เพิ่มจำนวน/คนทำ/วันส่งจริง ขั้นงานเหลือชื่อสถานะ ต้องจัดการถ้อยคำสั้น · `order.list` ส่งชื่อผู้เปิดออเดอร์เพิ่ม (คนทำของงานที่ยังไม่เข้าผลิต)
- ตรวจแล้ว: typecheck · lint 0 error · test 1866 ผ่าน · verify:ui ผ่าน · เรนเดอร์ชิ้นจริงด้วยข้อมูลตัวอย่างในเบราว์เซอร์ 1180px: เส้นสถานะกดกรอง ปฏิทินเลือกช่วง/ช่วงสำเร็จรูปคืนค่า YYYY-MM-DD ถูก · **ยังไม่ได้เปิดหน้าจริงที่ล็อกอินและยังไม่ลองบนฐานทดลอง**
- ติดอะไร: โมดูลผลิต — ใบ V2 ติ๊กในหน้าพิมพ์ DTF ไม่ได้ · หน้าเก่า `work-order-page`/`production-desk-*` ยังอยู่เพราะ `/proto` ใช้ · แจ้งเตือนร้านนอกต้องมี `CRON_SECRET` · ออเดอร์ — "คนทำ" ช่วงออกแบบเป็นผู้เปิดออเดอร์ ไม่ใช่กราฟิก (ไม่มีข้อมูลผู้รับงานออกแบบ)
- ทำต่อ: เบสเปิดหน้าออเดอร์/ผลิตจริง ลองกรอง/ปฏิทิน/ปิดขั้น/พิมพ์เสร็จ DTF/รับของร้านนอก → แก้ตามคำทัก → เบสอนุมัติ merge หน้าออเดอร์ขึ้น main
