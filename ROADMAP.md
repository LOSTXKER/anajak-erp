...
- ทำถึงไหน: 2026-09-15 หน้าออเดอร์อยู่ branch `nami/orders-mockup-20260914` (ยังไม่ merge main) · เบสบอก "ไม่เหมือน…รื้อเขียนใหม่" → ยก CSS ต้นแบบตรงตัวที่ `orders.module.css` + `c()` ใน `orders-ui.tsx` · เขียนใหม่รายการ/ดูย่อ/หัวใบ/ราง/7 แท็บ (`detail/order-*-tab.tsx`) · ตรรกะเงิน/สิทธิ์/สถานะ/V2 คงเดิม
...
- ทำต่อ: เบสเปิด `localhost:3000/orders` เทียบต้นแบบ → แก้ตามคำทัก → ลองบทบาทอื่น (ช่าง/บัญชี) และอัปไฟล์บนที่เก็บทดลอง → merge/ปล่อยตามอนุมัติ (ห้าม push main เอง)
