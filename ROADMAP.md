...
- ทำถึงไหน: 2026-09-15 ลงหน้าออเดอร์ตามต้นแบบรอบ 2 บน branch `nami/orders-mockup-20260914` (เบส "โอเคทำจริงเลย" · ยังไม่ merge main) · `/orders` = ราง pipeline กดกรอง+ตัวเลขเลยกำหนด · ตารางขั้นงาน/ต้องจัดการ (กฎเดียวกับหน้าแรกใน `src/lib/order-progress.ts`) · ดูย่อจากแถว ↑↓/Esc · การ์ดบนมือถือ · `/orders/[id]` = ป้ายต้องจัดการ/ติดด่านบนสุด · หัวมีรูปม็อกอัพ · รางแคปซูล · ภาพรวมซ้ายข้อมูล (วงแหวนกำหนดส่ง ไซซ์ แถบรับเงิน) ขวาม็อกอัพ · การ์ดเส้นเวลา · แท็บผลิต/เงิน/ไฟล์ 2 คอลัมน์ · ถอดตัวกรองสถานะ popover และแถบ flow เดิมของรายการ
- ตรวจแล้ว: typecheck · lint 0 error (22 warning เดิม) · test 188 ไฟล์/1,859 · verify:ui ผ่าน · เปิดจริงบนฐานทดลองผ่านแท็บ Chrome ของเบส: กรอง ดูย่อ ↑↓ Esc คืนโฟกัส ทุกแท็บของ ORD-2609-0026/0010 เส้นเวลา โหมดมืด · จอ 375 ดูจากภาพหน้าจริงแบบนิ่ง ไม่ล้นแนวนอน · console ไม่มี error · แก้ที่เจอตอนลอง: Esc พังเมื่อ target ไม่ใช่ element, จุดคั่นค้างท้ายบรรทัดบนมือถือ, คืนโฟกัสไม่รอเฟรม, เงินในภาพรวมเป็นทศนิยม 2 ตำแหน่งตามต้นแบบ
- ติดอะไร: รอเบสดูของจริงแล้วอนุมัติ merge main · ฐานทดลองไม่มีประวัติเปลี่ยนสถานะ จึงยังไม่เห็นบรรทัด "อยู่ขั้นนี้ N วัน" ในรางบนจอจริง · มือถือยังไม่ได้กดลองบนเครื่องจริง (ภาพนิ่งไม่มี JS) · แท็บรายการ/เงิน & บิลยังแสดงเงินทศนิยม 0–2 ตำแหน่งแบบเดิม · หน้าแรกบน production ยังไม่ได้เปิดหลังล็อกอิน
- ทำต่อ: เบสเปิด `localhost:3000/orders` (dev:demo) → แก้ตามคำทัก → merge/ปล่อยตามอนุมัติ (ห้าม push main เอง) → หน้าถัดไปของ A16 ตามที่เบสเลือก
