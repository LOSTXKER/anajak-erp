...
- ติดอะไร: ต้นแบบขัดข้อบังคับบางข้อจึงคงของจริงไว้ — เอกสารพิมพ์และลิงก์ลูกค้า (รายละเอียดใน SPEC) · ที่ยังไม่ทำเพราะต้องแตะ API/ด่านคู่กัน: ตัวเลขงานค้างของเมนู "งานของฉัน" · กล่องลอยแจ้งเตือนบนกระดิ่ง · หัวคอลัมน์ตารางพื้นเทา · โหมดมืดรางสว่างกว่าพื้น
- [ ] **งานแก้และเคลมหลังลูกค้ารับของ** (เบสสั่ง 2026-09-18) — **ก้อน 0 และ 1 ขึ้นเว็บแล้ว** (`7a8d5aa` · `081b680` ลงฐานจริง 18 ก.ย. 69 สำรองที่ `~/Backups/anajak-erp-db/`) · เทสบนเว็บจริงครบวงจร 19 ก.ย. เจอ 3 รู แก้แล้ว (`0178756`) · **ยังไม่มีใครใช้กับงานจริงสักใบ** (ฐานจริงยังว่าง) · **ก้อนถัดไป**: ตีกลับบางส่วน · แก้รายการใต้ใบเคลม · งานแก้บนใบผลิต (ดูข้อถัดไป) · คู่มือ 80 เคส: https://claude.ai/artifact/MtyRHyJgxr9KGxtvQhTHzY · **รอเคาะนโยบาย**: รับเคลมกี่วัน · ริบมัดจำ · คืนเงินหรือเครดิต · ค่าของที่ทำทิ้ง
- [ ] **ใบผลิตแบบหน้าออเดอร์ + ระบบปัญหาเดียว** (เบสสั่ง 2026-09-19 · เคาะต้นแบบ 09-20) — **ทำแล้วบน branch `nami/work-order-problems-20260919` รอเบสเปิดดูแล้วสั่งขึ้นเว็บ** (`ef6a376` `62677bc`) · การ์ดขั้น+แท็บสินค้าเป็นตารางรายรายการ ลายกรองตามขั้น ตัววาดใช้ร่วมกับหน้าออเดอร์ที่ `components/order-items` · แผ่นแจ้งปัญหาและกล่องตัดสินอยู่ในการ์ดขั้น · แจ้งได้ทุกชนิดขั้นแล้ว · ลบใบผลิตชุดเก่า 5 ไฟล์ ย้ายด่าน/เทสต์มาตรวจหน้าจริง · ตรวจครบและเปิดหน้าจริงเทียบต้นแบบแล้ว · ที่มาและแผน: สมอง `design-work-order-problems-2026-09-19.md`
- **ก้อนถัดไป (ยังไม่ทำ)**: จอช่าง `/production/floor` ยังใช้หน้าต่างชุดเดิม · **รออนุมัติ migration** ใบปัญหาเป็นแถวจริง (ต่อยอด `ProductionException` ไม่ใช่ `OrderClaim` — หน้าลูกค้าจะขึ้นแถบเคลมทันที) → ปัญหาแบบไม่หยุดทั้งขั้น · หลายเรื่องต่อขั้น · เวลา/ผู้แจ้ง/รูป · คิวปัญหารวม QC ร้านนอก เคลม · ต้องรวมตัวอ่าน "ขั้นมีปัญหา" เป็นฟังก์ชันเดียวก่อน
- ยังค้าง: ปุ่มย้อนหน้าผลิตไม่รู้จัก V2 (V2 ปิดอยู่ ไม่กระทบ) · เลขรายการในกล่อง error ไม่ขยับตาม · ลบชุดงานยังไม่มีถามยืนยัน · ชื่อบริการพิมพ์ในฐานจริงยังเป็นอังกฤษ ต้องแก้เองที่ ตั้งค่า → บริการ · ยังไม่มีใครพิมพ์ใบกำกับจริงดูสักใบ (นิติบุคคลที่ไม่มีชื่อผู้ติดต่อ)
