...
- [ ] **งานแก้และเคลมหลังลูกค้ารับของ** (เบสสั่ง 2026-09-18 · อนุญาตให้รื้อถึงราก) — **ก้อน 0 และก้อน 1 ขึ้นเว็บแล้ว** (`7a8d5aa` · `081b680` ลงฐานจริง 18 ก.ย. 69 สำรองที่ `~/Backups/anajak-erp-db/`) · ใช้ได้จริงแล้วทั้งเส้น (ใบเคลมรายไซซ์ · เปิดเองตอนตีกลับใบส่ง · ปุ่มเดียวสั่งงานแก้ · ด่านปิดใบ · สิทธิ์ `decide_claims` · แถบบนใบออเดอร์และหน้าลูกค้า) · **เทสบนเว็บจริงครบวงจร 19 ก.ย. 69** (ออเดอร์ทดสอบลบทิ้งแล้ว แก้ 3 รูที่เจอ) · **ยังไม่มีใครใช้กับงานจริงสักใบ** (ฐานจริงยังว่าง) · **ก้อนถัดไป**: ตีกลับบางส่วน (2) · แก้รายการใต้ใบเคลม (3) · งานแก้บนใบผลิต V2 (ต้องขึ้นเหตุผลบนจอ) · คู่มือ 80 เคส: https://claude.ai/artifact/MtyRHyJgxr9KGxtvQhTHzY · รอเคาะนโยบาย: รับเคลมกี่วัน · ริบมัดจำ · คืนเงินหรือเครดิต · ค่าของที่ทำทิ้ง
- ยังค้าง: ปุ่มย้อนหน้าผลิตไม่รู้จัก V2 (V2 ปิดอยู่ ไม่กระทบ) · เลขรายการในกล่อง error ไม่ขยับตาม · ลบชุดงานยังไม่มีถามยืนยัน · `work-order-page.tsx` เป็นเชลล์ที่ไม่ได้ใช้ รอเคาะว่าจะลบ · ชื่อบริการพิมพ์ในฐานจริงยังเป็นอังกฤษ ต้องแก้เองที่ ตั้งค่า → บริการ · ยังไม่มีใครพิมพ์ใบกำกับจากฐานจริงดูสักใบ

- ฐานทดลองพร้อมลองครบวงจรแล้ว (19 ก.ย. 69 · ชุดข้อมูลและวิธีเริ่มอยู่ README) — ที่ยังลองไม่ได้: บัญชีช่าง/หัวหน้าล็อกอินไม่ได้ (`supabaseId` ไม่ใช่ของจริง) · ผูก CN/DN กับใบเคลมจากจอไม่ได้ (`billing.create` ไม่รับ `claimId`) ไม่มีปุ่มยกเลิกใบเคลมและหน้ารวมคิวเคลม · ข้อมูลอิงวันที่ seed เว้นเกิน 3 วันกลายเป็นงานติดหล่มทั้งกระดาน ให้ seed ใหม่ก่อนลองรอบถัดไป

### ขอบเขตและจุดส่งต่อ A16

**ขอบเขตรอบ refactor ชุดกลาง (เบสอนุมัติ 2026-09-15):** ย้ายชิ้นส่วนที่เคาะแล้วขึ้น `components/kit` โดยหน้าตาไม่เปลี่ยน · หน้าอื่นย้ายทีละหน้าหลังเบสเคาะหน้าลองของหน้านั้น ไม่ย้ายเงียบ · ไม่รวม schema/สิทธิ์/สูตรเงิน/สถานะ
