...
- ทำถึงไหน: 2026-09-15 หน้าออเดอร์อยู่บน branch `nami/orders-mockup-20260914` (ยังไม่ merge main) · เบสเปิดของจริงสองรอบแล้วบอก "ไม่เหมือน" → "รื้อเขียนใหม่ refactor ไปเลย" → รื้อทั้งสองหน้าโดยยก CSS ต้นแบบมาตรงตัวที่ `orders.module.css` + `orders-ui.tsx` (`c()` ชื่อคลาสเดียวกับต้นแบบ) · รายการ/ดูย่อ/ราง pipeline/หัวใบ/รางสถานะ/แท็บ 7 แท็บเขียนใหม่ทั้งหมด (รายการ งานผลิต จัดส่ง เงิน & บิล ม็อกอัพ & ไฟล์ แยกไฟล์ `detail/order-*-tab.tsx`) · ตรรกะเงิน/สิทธิ์/สถานะ/ด่านพร้อมผลิต/V2 คงเดิม
- ตรวจแล้ว: typecheck · lint 0 error (22 warning เดิม) · test 188 ไฟล์ · verify:ui ผ่าน (แก้สัญญาที่ผูกคลาสเก่า + stub `.css` ให้ด่าน render) · วัดกล่องหน้าจริงเทียบต้นแบบที่จอเดียวกัน ต่างไม่เกินไม่กี่ px ครบรายการ/หัวใบ/ภาพรวม · เปิดครบ 7 แท็บเทียบต้นแบบ (ออเดอร์ผลิตอยู่ + ส่งแล้ว) · โหมดมืด · 375px ไม่ล้นทุกแท็บ (ภาพนิ่ง) · console ไม่มี error
- ติดอะไร: ยังไม่ได้กดธุรกรรมจริงบนแท็บใหม่ (อัปไฟล์ ลบไฟล์ ออกบิล รับเงิน สร้าง/ส่งใบส่งของ ตรวจรับ QC) · กลุ่มไฟล์ "ทั่วไป" อัปเป็น PHOTO/PO_DOCUMENT รอเบสยืนยัน · ต่างจากต้นแบบโดยตั้งใจ: ไม่มีใบปะหน้า (ไม่มี route) ไม่มีส่งลิงก์บิล ไม่มีปุ่มเริ่มผลิตในหน้าออเดอร์ เส้นรางไม่ลากผ่านแคปซูล · ของเดิมที่หน้าออเดอร์ไม่ใช้แล้วแต่ยังเก็บไว้: `order-delivery-section.tsx` `order-files-card.tsx` `MockupPanel` · มือถือยังไม่ได้ลองบนเครื่องจริง
- ทำต่อ: ลองธุรกรรมบนแท็บใหม่ใน demo หลายบทบาท → เบสเปิด `localhost:3000/orders` เทียบต้นแบบ → แก้ตามคำทัก → merge/ปล่อยตามอนุมัติ (ห้าม push main เอง)
