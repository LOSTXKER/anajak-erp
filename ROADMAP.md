...
- [ ] หลังเคาะ: ลงของจริง (server ที่ต่อได้เลย: `factory.stationQueue` · `production.updateStep/assignStep/reportStationProblem/resolveStationProblem` · V2 `manufacturing.station*`) · จอใช้ร่วมกันหลายคน = PIN ต่อคน (งานฝั่ง auth — ถามก่อน) · ลำดับที่หัวหน้าจัด (ทาง B) ต้องเพิ่มที่เก็บ (ถามก่อน)
