...
### รอทบทวนต้นแบบออเดอร์ครบทุกแท็บ

- คำขอล่าสุด 13 ก.ย. 2569: จัดแท็บที่เหลือใน `/proto/order-workspace` และแก้จุดกดที่ซ่อนความสามารถแล้ว. ผู้ติดต่อแสดงโทรศัพท์ LINE และอีเมลพร้อมปุ่มคัดลอก/ข้อมูลลูกค้า; จำนวน ยอดเงิน และจัดส่งมีปุ่มระบุปลายทาง. มือถือเห็นทั้ง 7 แท็บเป็นสองแถว พื้นโปร่ง เส้นใต้แท็บที่เลือก
...
- แท็บที่เหลือ: `?tab=production` แสดงสินค้า/ไซซ์/งานพิมพ์คู่กับสถานะม็อกอัพและไฟล์; `?tab=delivery` แสดงผู้รับ/ที่อยู่คู่กับรายการแพ็กและตัวอย่างใบปะหน้า; `?tab=history` เรียงเหตุการณ์ตามวัน กรองข้อมูล/ไฟล์/เงิน เปิดส่วนที่เกี่ยวข้องได้ และเห็นค่าก่อน–หลังเมื่อแก้ข้อมูล. จำนวนและเหตุการณ์มาจากข้อมูลตัวอย่างและการกดในหน้าลอง ไม่สมมติว่างานผลิตเริ่มหรือจัดส่งแล้ว
...
- ตรวจรอบนี้: ดูงานผลิต/จัดส่ง/ประวัติ/ผู้ติดต่อบน 1440/390 และ Light/Dark, Prompt เดิม รูปโหลดครบ หน้าจอไม่ล้น; คัดลอกโทรศัพท์แล้ววางจริงในช่อง PO ก่อนยกเลิก; แก้ที่อยู่แล้วใบปะหน้า/ภาพรวม/ประวัติตรงกัน; เติมไฟล์พิมพ์แล้วจำนวนและประวัติปรับ; รับเงินตัวอย่าง 100 บาทแล้วมีเหตุการณ์เดียว; ตัวกรองประวัติคงเมื่อเปลี่ยนแท็บและแสดงทางกลับเมื่อหมวดว่าง. ลองกรณีไม่มีม็อกอัพ/ไฟล์และเปิดใบสั่งงานแล้ว; ไม่พบ console error/warning. ตรวจ React และคำบนจอตาม ui-guidance แล้ว คืนหน้าลองเป็น Light และขนาดปกติ
- ด่านตรวจรอบนี้: typecheck ผ่าน; lint 0 errors/22 warnings เดิม; 201 files/1,971 tests ผ่าน; verify:ui 255 + work-order 32 ผ่าน. หลังเก็บตำแหน่งปุ่ม/แท็บมือถือ ตรวจ typecheck และ lint ไฟล์ที่แก้ซ้ำผ่าน. Log `remaining-tabs-*` อยู่ `.tmp-test/ui-polish-20260913/` ไม่เข้า Git. สคริปต์ proto-shots เดิมไม่พบทั้งต้นฉบับและสำเนาติดตั้ง จึงตรวจภาพผ่าน browser โดยไม่ได้อ้างผลผ่านสคริปต์เก่า
