...
### รอเบสเคาะต้นแบบแนว Notion ของหน้าแรกและรายละเอียดออเดอร์

- คำขอ 13 ก.ย. 2569 (ย้ายจาก Codex มา Claude Code): ทำต้นแบบใหม่ของหน้ารายละเอียดออเดอร์และหน้าแรก “แบบ Notion — minimal แต่ดูง่าย ไม่รก” ใช้สกิล ui-guidance. ทำเป็นไฟล์ HTML เดี่ยวกดเล่นได้ เผยแพร่เป็น Artifact ส่งลิงก์ให้เบส และเก็บไฟล์ไว้ในสมอง `records/projects/anajak-erp/mockup-order-home-notion-2026-09-13.html` (ยามกลางไม่ให้เพิ่มไฟล์ใหม่ใต้ `docs/` ของ repo นี้); ไม่แตะโค้ดจริงและไม่เพิ่มหน้าใต้ `/proto`
- โครงที่เลือกในต้นแบบ: พื้นขาว Prompt เดิม เมนูซ้ายสีเทาอ่อนแบบ Notion ชื่อออเดอร์เป็นหัวใหญ่ + ป้ายสถานะ ข้อมูลเป็นแถว “คุณสมบัติ” (ไอคอน–ป้าย–ค่า) ตัวเลขสำคัญ 3 ตัว (กำหนดส่ง จำนวน ยอดรวม) เด่นด้วยขนาด กรอบเส้นบางเฉพาะสองพื้นที่ทำงาน (ข้อมูลซ้าย–ม็อกอัพขวา) ป้ายแจ้งเตือนบนสุด รางสถานะ 1–11 + บรรทัดขั้นต่อไป แท็บเส้นใต้ 7 แท็บเดิม ผู้ติดต่อเห็นโทร/LINE/อีเมลทันที ไม่มีปุ่มคัดลอกและไม่มี CTA ซ้ำ. หน้าแรก: สรุป 3 ตัวเลข → คิวออเดอร์เป็นตารางกรอง/ค้นหา/กางแถว/เปิดออเดอร์ได้ทุกแถว → ฝั่งขวารอตัดสินแบบและกำหนดส่งรายวัน
- ทดลองได้ในต้นแบบ: ออเดอร์จำลอง 6 ใบ (ข้อมูลเต็ม 1 ใบ) เปิดจากคิวได้ทุกใบ; แก้ข้อมูล/ที่อยู่แล้วเห็นค่าก่อน–หลังในประวัติ; รับเงินแล้วยอดค้าง/ใบเสร็จ/ประวัติปรับตาม; เพิ่มม็อกอัพ/ไฟล์พิมพ์ตัวอย่าง; มุมมองลูกค้ากดอนุมัติ/ขอแก้ไขแล้วสถานะและขั้นต่อไปเปลี่ยน; สลับกรณี มีม็อกอัพ/ยังไม่มี/ขาดไฟล์พิมพ์; สลับสว่าง/มืด; ปุ่ม “โครงปัจจุบัน” เปิดหน้าจริงบน localhost ไว้เทียบ. ทุกอย่างเป็นสถานะชั่วคราวในหน้า ไม่ต่อฐาน ไม่ส่งข้อความ
- ตรวจแล้ว 13 ก.ย.: เปิดผ่าน dev server (`public/upload/` ชั่วคราว ลบแล้ว) บน Chrome 1528 และ Browser pane 390 ทั้งสว่าง/มืด ไม่ล้นแนวนอน ไม่มี console error; กดครบ 7 แท็บ dialog ทุกแบบ และทางสำเร็จ/ติดขัดข้างต้น. เกณฑ์ตามที่เบสเคยทัก: ไม่มี CTA ซ้ำ ผู้ติดต่อไม่ซ่อน โฟกัสด้วยขนาด/น้ำหนัก ข้อความเหลือชื่อ ตัวเลข สถานะ และเหตุที่กดไม่ได้
- จุดต่อ: รอเบสเคาะทิศ (ชอบ/ไม่ชอบส่วนไหน) แล้วค่อยนำโครงที่เลือกไปต่อกับหน้าออเดอร์/หน้าแรกจริงตามขอบเขตที่อนุมัติ; ต้นแบบ React เดิมใต้ `/proto/order-workspace` ยังอยู่เป็นตัวเทียบ. งานอยู่ branch `codex/ui-focus-two-pages-20260913` ยังไม่ใช่ main/Production
