...
export const config = {
  ...
  // (/approve, /upload — ลูกค้าไม่มี session ไม่ต้องรีเฟรช) · body มี guard ไม่ redirect /api/*
  matcher: [
    "/((?!_next/static|_next/image|favicon.ico|approve/|upload/|.*\\.(?:svg|png|jpg|jpeg|gif|webp|ico)$).*)",
  ],
};
