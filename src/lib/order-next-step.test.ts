...
describe("getOrderNextStep — จุดโฟกัสเดียวต่อสถานะ", () => {
  ...
  it("ด่านตรวจ → ชี้ไปการ์ดตรวจนับ QC ไม่ใช่ปุ่มเดินสถานะข้ามด่าน (Gate B4)", () => {
    const step = getOrderNextStep({ ...base, internalStatus: "QUALITY_CHECK" });
    expect(step?.action).toEqual({ type: "ANCHOR", target: "qc" });
    expect(step?.title).toContain("นับ");
  });

  it("กำลังแพ็ค → เดินสถานะไปพร้อมส่งตามเดิม", () => {
    expect(getOrderNextStep({ ...base, internalStatus: "PACKING" })?.action).toEqual({
      type: "STATUS",
      to: "READY_TO_SHIP",
    });
  });
