...
/** ผิวช่องกรอก/ช่องเลือก/กล่องข้อความ — พื้น สีตัวอักษร ครบทั้งสว่างและมืด
 *  (ไม่รวมมุมโค้งกับความสูง — สองอย่างนั้นต่างกันตามทรงและตำแหน่งที่ใช้)
 *
 *  **ไม่มีขอบ** (เบสเคาะ 2026-08-03 "อยากลดการใช้เส้น ขอบทั้งเว็บ ให้มัน minimal"):
 *  ช่องกรอกบอกตัวเองด้วย "พื้นที่จมลงไป" แทนเส้นรอบตัว — จอฟอร์มเดียวเคยมีเส้น 133 เส้น
 *  เพราะการ์ด/กล่องย่อย/ตาราง/ทุกช่องกรอกต่างมีขอบของตัวเอง ซ้อนกัน 4 ชั้น
 *
 *  ยังคง `border` ไว้ (ความหนา 1px สีโปร่ง) ไม่ใช่ border-0 — เพราะ FOCUS_FIELD
 *  เปลี่ยน "สีขอบ" ตอนโฟกัส ถ้าไม่มีความหนาไว้ก่อน วงแหวนโฟกัสจะหายไปทั้งระบบ
 *  และความสูงของ control จะขยับ 2px ตอนโฟกัส
 *
 *  พื้น light = slate-100 (จมกว่าการ์ดขาว) · dark = slate-950 (จมกว่าการ์ด #252528)
 *  → กล่องย่อยในการ์ดที่มีช่องกรอกอยู่ข้างในต้องใช้ SUNK_PANEL (จางกว่า) ไม่ใช่ slate-100
 *    ไม่งั้นช่องกรอกจะกลืนหายไปกับพื้นกล่อง */
export const FIELD_SURFACE =
  "border border-transparent bg-slate-100 text-slate-900 placeholder:text-slate-400 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500";

/** ช่องกรอกในตารางแก้ข้อมูล — โปร่งสนิท เห็นพื้นเฉพาะตอนชี้/โฟกัส
 *  ใช้เมื่อมีช่องกรอกหลายช่องเรียงในแถวเดียว (ตารางสินค้า/ลาย/ส่วนเสริม) —
 *  ถ้าใช้ FIELD_SURFACE เต็มจะได้ "ก้อนเทา 4 ก้อน" ในแถวเดียว แย่งสายตากับข้อมูลจริง */
export const FIELD_SURFACE_QUIET =
  "border border-transparent bg-transparent text-slate-900 placeholder:text-slate-400 hover:bg-slate-100 dark:text-slate-100 dark:placeholder:text-slate-500 dark:hover:bg-white/[0.06]";

/** กล่องย่อยที่จมลงไปในการ์ด (กลุ่มฟอร์ม · แถบสรุป) — จางกว่าผิวช่องกรอกหนึ่งขั้น
 *  เพื่อให้ช่องกรอกที่วางอยู่ข้างในยังลอยขึ้นมาเห็นได้ทั้งสองธีม */
export const SUNK_PANEL = "bg-slate-50 dark:bg-white/[0.04]";
