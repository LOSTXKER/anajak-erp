 ...
 *
 *  **พื้นขาว + เส้นบาง 1 เส้น** (เบสสั่ง 2026-08-03 "บางอย่างที่มันต้อง fill ข้อมูล
 *  ไม่ต้องซ่อน ช่อง user จะได้รู้ว่าช่องไหนต้องใส่ · ช่องบางช่องถูกกลืนไปกับพื้นหลังสีเทา")
 *
 *  ประวัติ: รอบ "ลดเส้นทั้งเว็บ" เคยถอดขอบออกแล้วใช้พื้นเทาจม (slate-100) แทน —
 *  ได้จอที่สะอาดขึ้นจริง แต่พังตรงที่ **ช่องกรอกกลืนกับพื้น**: เทียบสีแล้วต่างกัน
 *  แค่ 1-6 ขั้นเมื่อวางบนพื้นหน้า (#f3f4f6) หรือกล่องย่อย (slate-50) → คนไม่รู้ว่า
 *  ตรงนั้นกรอกได้ · พื้นขาวต่างจากทั้งพื้นเทาและกล่องย่อยเสมอ ไม่ว่าวางที่ไหน
 *
 *  เส้นที่ลดไปรอบก่อนยังลดอยู่ — ที่ถอดคือขอบของ **กล่องครอบ** (การ์ด · กรอบตาราง ·
 *  กล่อง "รายการที่ N" · เส้นคั่นแถว) ซึ่งเป็นตัวที่ซ้อนกัน 4 ชั้น ไม่ใช่ขอบช่องกรอก */
 ...
 *  ช่องกรอกเป็นพื้นขาวแล้ว กล่องย่อยจึงเข้มได้เต็มขั้น — ช่องข้างในยิ่งเด่นขึ้น */
export const SUNK_PANEL = "bg-slate-100 dark:bg-white/[0.05]";
