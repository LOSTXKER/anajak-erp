 ...
 *
 *  **ไม่มีเส้นขอบ — เห็นว่าเป็นช่องด้วยพื้นที่ตัดกับที่ที่มันยืน** (เบสเคาะ 2026-08-04
 *  "คือไม่ต้องมีเส้นเยอะ" หลังลองมาแล้ว 2 ทาง):
 *    รอบ 1 พื้นเทาจาง slate-100 ไม่มีขอบ → กลืนพื้น (ต่างจากพื้นหน้า/กล่องย่อยแค่ 1-6 ขั้น)
 *    รอบ 2 พื้นขาว + ตีเส้นทุกช่อง → เห็นชัดแต่เส้นกลับมารกทั้งจอ
 *  ทางที่ลงตัว: พื้น `--field-bg` (globals.css) ที่ **บริบทเป็นคนเลือกสี** —
 *  บนการ์ดขาวเป็นเทาจม #eceff2 (เข้มพอไม่กลืน) · ใน .sunk-panel สลับเป็นขาวเอง
 *  → ช่องเห็นชัดทุกที่ โดยไม่มีเส้นสักเส้น
 *
 *  ยังคง `border border-transparent` ไว้ — FOCUS_FIELD เปลี่ยนสีขอบตอนโฟกัส
 *  ถ้าไม่มีความหนาไว้ก่อน วงแหวนโฟกัสหาย และความสูง control ขยับ 2px */
export const FIELD_SURFACE =
  "border border-transparent bg-[var(--field-bg)] text-slate-900 placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500";
