          ...
          {(queue.isError && !queue.data) || (runs.isError && !runs.data) ? (
            ...
            <div className="space-y-6">
              <section aria-labelledby="dtf-active-title">
                ...
                {activeRuns.length === 0 ? (
                  <p className="mt-3 rounded-xl bg-surface-muted px-4 py-3 text-sm text-muted">
                    ยังไม่มีรอบค้าง
                  </p>
                ) : (
                  <ul className="mt-3 space-y-2">
                    {activeRuns.map((run) => {
                      ...
                      return (
                        <li
                          key={run.id}
                          className="flex flex-col gap-3 rounded-xl border border-divider p-4 sm:flex-row sm:items-center sm:justify-between"
                        >
                          <div>
      ...
      <DialogContent className="max-h-[94dvh] overflow-y-auto sm:max-w-3xl">
        ...
        <div className="space-y-3">
          {items.map((item) => {
            ...
            const totals = itemTotals(value);
            return (
              <fieldset key={item.id} className="rounded-xl border border-divider p-4">
