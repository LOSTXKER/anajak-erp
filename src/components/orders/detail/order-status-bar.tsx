...
/* วงกลมโตขึ้นจาก 18px เป็น 24px ตอนใส่ตัวเลขข้างใน (เบสสั่ง 2026-08-30
   ...
   รัศมี 12px ถูกเขียนซ้ำอีก 3 ที่ในไฟล์ (ทั้งหมดเป็น inline style ข้างล่าง)
   แก้ขนาดวงเมื่อไหร่ต้องไล่แก้ให้ครบ:
     · top ของเส้นเชื่อม = รัศมี ลบครึ่งความหนาเส้น (12 − 1 = 11)
     · ปลายเส้นของขั้นหัว-ท้าย = รัศมี (เส้นจบตรงจุดกึ่งกลางวง)
        ...
        {flowSteps.map((step, i) => {
          ...
          return (
            <li
              ...
              /* ── เรขาคณิตของรางเป็น inline style ไม่ใช่คลาส Tailwind ค่าเฉพาะ ──
                 เบสเจอกับตาสองรอบ (2026-08-30 "ทำไมเส้นไม่ต่อกัน" · "ทำไมเส้นยังยาว
                 ไม่สมมาตร") ว่าเครื่องที่ไฟล์ CSS ยังไม่อัปเดตจะได้รางเพี้ยน เพราะคลาส
                 Tailwind ที่ใส่ค่าเฉพาะลงในวงเล็บเหลี่ยม (flex-basis ของช่อง · ปลายเส้น
                 ที่เป็น calc) มีผลก็ต่อเมื่อ CSS ถูก generate มาแล้ว
                 · inline style มากับตัวหน้าเสมอ จึงเพี้ยนไม่ได้
                 (สี/ตัวอักษรยังเป็นคลาสตามปกติ — พวกนั้นใช้ค่ามาตรฐานที่มีอยู่แล้วทั้งเว็บ)

                 สูตรช่อง: หัว/ท้าย = ครึ่งช่อง + รัศมีจุด (`flex: .5 1 12px`)
                 → จุดหัวอยู่ 12px จากขอบซ้าย · จุดท้าย 12px จากขอบขวา
                 · ทุกช่วงกว้างเท่ากันพอดี (วัดจริงจอ 1440: 133-135px ทุกช่วง)
            ...
            >
              {/* เส้นเชื่อม = ชิ้นเดียวต่อหนึ่งช่วง (เคยแตกเป็นสองครึ่งแล้วเห็นเส้นขาด)
                  ปลายซ้ายอยู่ที่ -50% ของตัวเองเสมอ = จุดของขั้นก่อนหน้าพอดี ทั้งกรณี
                  ขั้นก่อนเป็นช่องกลาง (กว้าง s จุดกึ่งกลาง → -s/2) และเป็นขั้นแรก
                  (กว้าง 12+s/2 จุดอยู่ที่ 12 → -s/2 เท่ากัน)
                  ยกเว้นขั้นสุดท้ายที่ช่องแคบกว่า จึงคิดจากขอบซ้ายตัวเอง: -100%+12px */}
              {!isFirst && (
                <span
                  aria-hidden="true"
                  style={{
