...
export function OrderOverviewTab({
  ...
  return (
    <div className="space-y-5">
      {/* อ่านใบงานให้รู้เรื่องก่อน — หัวหน้าและ status rail บอกตัวตน/ขั้นงานแล้ว
          การ์ดนี้จึงตอบเฉพาะ ลูกค้า · ส่งเมื่อไร · กี่ชิ้น · ยอดเท่าไร */}
      <Section
        data-order-overview-card="summary"
        title={<SectionTitle icon={Info}>สรุปออเดอร์</SectionTitle>}
      ...
      >
        <div className="space-y-5">
          ...
          >
            <SummaryFact label="ลูกค้า" detail={customer?.company || undefined}>
              {customer ? (
                customer.name
              ) : (
          ...
          <Group label="ข้อมูลประกอบ" divided>
          ...
          {/* เวลาเป็นข้อมูลอ้างอิง ไม่ใช่หัวใจของการตัดสินใจ จึงอยู่บรรทัดท้ายแบบสงบ */}
          <dl className="flex flex-wrap gap-x-5 gap-y-1 border-t border-divider pt-4 text-xs text-muted">
            {creatorName && (
            ...
            <div className="space-y-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-lg font-semibold text-strong [overflow-wrap:anywhere]">
                    {customer.name}
                  </p>
                  {customer.company && (
