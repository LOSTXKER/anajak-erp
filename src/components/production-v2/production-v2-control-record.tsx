...
function OperationLedger({
  ...
  return (
    <Section title="เส้นทางการผลิต" description="สถานะและผลผลิตของทุกขั้น เรียงตามคิวที่หัวหน้ากำหนด" action={<Route className="h-5 w-5 text-muted" aria-hidden />}>
      {workOrder.operations.length === 0 ? (
        ...
        <ol className="space-y-1">
          {workOrder.operations.map((operation, index) => {
            ...
            return (
              <li key={operation.id} className="relative grid gap-3 rounded-xl px-3 py-4 hover:bg-interactive-hover sm:grid-cols-[2rem_minmax(0,1fr)_auto]">
...
function ReferenceControl({ workOrder }: { workOrder: WorkOrder }) {
  ...
  return (
    <Section
      ...
      description="สำเนาที่ใช้กับใบสั่งผลิตนี้ จะไม่เปลี่ยนตามต้นฉบับภายหลัง"
...
function QuantityLedger({
  ...
  return (
    <Section
      ...
      description="แต่ละกลุ่มเป็นยอดของขั้นงานและศูนย์งานนั้น ไม่ใช่ยอดรวมซ้ำของทั้งใบ"
      flush
    >
...
function ExceptionLedger({
  ...
  return (
    <Section
      ...
      description="ติดตามปัญหา แนวทางจัดการ และผลการแก้ไข"
    >
      <ul className="space-y-3">
        {exceptions.map((item) => {
          ...
          return (
            <li key={item.id} className="rounded-xl bg-surface-muted p-4">
...
function ReworkLedger({
  ...
  return (
    <Section
      title="งานส่งแก้"
      description="ทุกชิ้นที่ส่งแก้ต้องกลับมาตรวจซ้ำก่อนนับเป็นงานดี"
      ...
      <ul className="space-y-3">
        {cases.map((item) => {
          ...
          return (
            <li
              key={item.id}
              className="flex flex-col gap-3 rounded-xl bg-surface-muted p-4 sm:flex-row sm:items-start sm:justify-between"
            >
              <div>
                <StatusLabel label={state.label} tone={state.tone} />
...
function EventLedger({
  ...
  return (
    <Section
      ...
      description="บันทึกย้อนหลังที่เพิ่มต่อเนื่องและไม่แก้ทับ"
    >
