...
const COPY: Record<Variant, { name: string; idea: string; summary: string; tradeoff: string }> = {
  ...
  one: {
    ...
    tradeoff:
      "หัวหน้าเห็นภาพรวม 7 ขั้นแค่จากแถบเล็ก ต้องกดทีละขั้นเพื่อดูยอด/คน (หรือกดกางตารางท้าย) · ใกล้หน้างานของช่างมาก จนหัวหน้ากับช่างเห็นหน้าเดียวกัน — ถ้าเบสชอบ อาจยุบหน้างาน /production/floor รวมกับใบผลิตได้ (งานใหญ่) · ต่างจาก “B ตอนนี้ทำอะไร” ที่เคยเสนอ 2 ก.ย. ตรงที่ไม่มีรายการขั้นข้าง ๆ เลย · **แถบขั้น 1–7 เรียงเป็นทางเดียว ทั้งที่เสื้อ/ฟิล์ม/ร้านนอกเดินพร้อมกันจริง** (เบสทัก 6 ก.ย. “มันจะมีทางขนาน”)",
  },
  flow: {
    name: "E · ตอนนี้ทำอะไร (รู้ทางขนาน) — wizard ที่หน่วยคือ “สิ่งที่ทำได้ตอนนี้” ไม่ใช่ “ขั้นที่ N”",
    idea: "เบสอยาก wizard แต่ห่วงทางขนาน → ให้ wizard เดินตาม “สาย” ไม่ใช่ตามเลขขั้น",
    summary:
      "บนสุดเป็นแผนที่เส้นทาง: สายที่เดินพร้อมกัน (เสื้อ · ฟิล์ม DTF · ร้านปัก · ร้านป้ายคอ) อยู่คนละแถว เส้นวิ่งมารวมที่ขั้นที่ต้องรอกัน (รีดร้อนรอเสื้อ+ฟิล์ม · QC รอทุกสาย) · ใต้แผนที่ = การ์ด “ตอนนี้ทำได้” สายละใบวางคู่กัน แต่ละใบมีปุ่มเดียว (ติดปัญหาแยกกลุ่มขึ้นก่อน) · ขั้นที่ยังไม่ถึงคิวเป็นบรรทัดสั้น “รอ X + Y” · กดขั้นไหนในแผนที่ = เปิดการ์ดขั้นนั้น · เสื้อ/ลายเป็นแถบย่อ ตารางทั้งใบพับไว้ท้าย · กติกา “ทำได้ตอนนี้” = ของจริงที่ใบผลิตใช้อยู่ (ขั้นแรกที่ยังไม่ปิดของแต่ละสาย) ไม่ได้คิดใหม่",
    tradeoff:
      "ต้องอ่านแผนที่เส้นทางเป็น (มีสูงสุด 4 แถว) — ใบที่ทำเองทั้งใบเหลือ 2 แถวเท่านั้น · ใบที่ติดปัญหา 2 สายพร้อมกัน (อย่างใบตัวอย่างนี้) จะมีการ์ด 4 ใบบนจอ ไม่ใช่ “ปุ่มเดียว” แบบ D · แผนที่กว้าง บนมือถือต้องเลื่อนซ้ายขวาในกรอบ · ยังไม่ได้ตอบว่าแท็บ ข้อมูลใบ/ประวัติ ไปอยู่ไหน (เหมือน C/D)",
  },
};
...
const NOTES = [
  ...
  "กดเลือกขั้นได้ทุกขั้น (ปัจจุบัน = รายการซ้าย · D = แถบบน · E = แผนที่) — ลองกด “พิมพ์ฟิล์ม DTF” (ปิดแล้ว) กับ “รีดร้อน” (ตามกระดาษ) เทียบ",
  "E เพิ่ม 6 ก.ย. ค่ำ หลังเบสถาม “ทำแบบ wizard ได้มั้ย แต่มันจะมีทางขนาน” — ใบตัวอย่างนี้มี 4 สายเดินพร้อมกัน (เสื้อติดปัญหา · ฟิล์มเสร็จ · ปักอยู่ที่ร้าน · ป้ายคอเลยนัด) จึงเป็นเคสยากสุดของ E: กด “ทำเองทั้งใบ” เพื่อดูตอนเหลือ 2 สาย",
] as const;
