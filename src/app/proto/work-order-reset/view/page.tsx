...
export default function ResetFullPage() {
  ...
  return <AppShell><div className="space-y-6"><div className={cn("card-surface flex items-center justify-between gap-3 px-4 py-1 sm:px-5", RADIUS.surface)}><Link href={`/proto/work-order-reset?v=${variant}&case=${scenario}&boss=${boss ? 1 : 0}`} className="inline-flex min-h-11 items-center gap-2 text-sm text-secondary"><ArrowLeft className="size-4" />เทียบแบบ</Link><span className="text-xs text-secondary">หน้าลอง / ข้อมูลจำลอง</span></div><Preview variant={variant} scenario={scenario} boss={boss} idPrefix="full" /></div></AppShell>;
}
