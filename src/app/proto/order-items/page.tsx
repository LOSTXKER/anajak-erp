...
const COPY: Record<Variant, { name: string; idea: string; summary: string; tradeoff: string }> = {
  current: {
    name: "ปัจจุบัน — แบบ A ที่ลงของจริงแล้ว (เบสเคาะ 2026-09-06)",
    idea: "ปุ่มนี้ชี้ไปที่ component ตัวจริง จึงเปลี่ยนตามของจริงเสมอ",
    summary:
      "นี่คือ `OrderItemsDisplay` ตัวจริงของแท็บรายการ ต่างจากหน้าจริงแค่ข้อมูลปลอม — ตั้งแต่ 6 ก.ย. เย็น มันคือแบบ A ที่ลงของจริงแล้ว (ต่างจาก A ข้าง ๆ ตรงที่กล่องเสื้อลูกค้าใช้ฟอร์มตรวจรับตัวจริง) · หน้าตาเดิมก่อนแก้ (เสื้อแต่ละตัวเป็นก้อน ตารางย่อยหลายชั้น “รวม 1” โผล่หลายที่) ดูย้อนได้จากประวัติ Git เท่านั้น",
    tradeoff:
      "หน้าลองนี้ไม่มี “ของเดิมก่อนแก้” ให้กดเทียบอีกแล้ว — ถ้าต้องการดูให้เปิด git show 88a8ba5:src/components/orders/detail/order-items-display.tsx",
  },
  form: {
...
const OUT_OF_SCOPE = [
  ...
  "รหัสประเภทส่วนเสริม (SIZE_LABEL · NECK_LABEL) เคยโชว์เป็นคอลัมน์ในทุกแบบ — เบสเคาะ 09-06 ดึกว่าซ้ำกับชื่อ ตอนนี้ทั้งฟอร์มและหน้าดูเหลือชื่อเดียว รหัสตามแค็ตตาล็อกไปเงียบๆ",
