...
const NOTES = [
  "รอบ 11 (8 ก.ย. ดึก) เบส “ฉันรู้ละ”: 2 แท็บ — ขั้นตอน (ซ้าย = ตารางเช็ครายตัวแถวละไซซ์ กรอกจำนวนได้ · ขวา = เช็คลิสต์ + ข้อมูลออเดอร์) · สินค้า (เหมือนหน้ารายการออเดอร์) · คิดต่อให้: ช่องกรอกต่างกันตามขั้น (เบิก/รับเสื้อ = ได้จริง · พิมพ์ฟิล์ม = พิมพ์แล้ว · รีดร้อน = ทำแล้ว+เสีย · ร้านนอก = ส่งไป+รับกลับ · QC = ผ่าน+เสีย · แพ็ก = แพ็กแล้ว) · แถวครบขึ้น ✓ เอง · ปุ่ม “ครบทุกแถว” กดทีเดียวบนจอทัช · ยอดรวมของตารางกลายเป็นยอด “ทำแล้ว” ของขั้น · ยังขาดกี่ตัวขึ้นชิปเตือนที่การ์ดเช็คลิสต์ (เตือน ไม่กั้น — ของเสียมีจริง) · ของจริง: ช่องกรอกต่อขั้นต้องตั้งในสูตรขั้นงาน + เก็บยอดรายไซซ์ต่อขั้น (schema ⚠️)",
  "รอบ 10 (8 ก.ย. ดึก): เบส “UI ไม่ต้องอธิบายละเอียด มันรก คนใช้ไม่ได้โง่” (= กติกา A8 ระดับ 1 ไม่ต้องบอก) → ถอดกล่องรายละเอียดขั้น · ตัดป้ายกำกับ/ประโยคบอกวิธีใช้/hint ในเมนู/tooltip ปุ่ม ทิ้งหมด เหลือชื่อ ตัวเลข สถานะ และเหตุที่กดไม่ได้เท่านั้น",
  "รอบ 9 (8 ก.ย. ดึก): เบส “แสดงข้อมูลเฉพาะขั้นตอนนั้น ๆ · ไม่ต้องมีแถบแยก · ลายเสื้อเด่นสุด ให้ฝ่ายผลิตรู้ว่าต้องทำเสื้ออะไร · เช็คลิสต์ขวา · ข้อมูลออเดอร์ขวา” → ไม่มีแท็บ ไม่มีแถบข้าง: ซ้าย = ขั้นที่ยืนอยู่ (ชื่อใหญ่ · ผู้ทำ · ทำแล้ว · ควรเสร็จ · ร้าน) แล้วเสื้อทีละตัว (ม็อกอัพใหญ่ · ลายที่เป็นงานของขั้นนี้เด่น ลายอื่นจาง · ตารางไซซ์) · ขวา = เช็คลิสต์ก่อนปิดขั้น → ข้อมูลออเดอร์ · ประวัติย้ายไปเมนู ⋯ · ทุกขั้นดูได้จากรางเท่านั้น (ไม่มีรายการขั้น)",
  "รอบ 8 (8 ก.ย. ดึก): เบส “ขั้นตอนเป็น sidebar กดแล้วแสดง checklist · ข้อมูลใบไว้ขวาสุด · รวม 3 คอลัมน์ · เอาร้านนอกในใบนี้ออก” → ซ้าย = ขั้นตอน (กดกาง/หุบเช็คลิสต์ · ขั้นที่ยืนอยู่กางไว้ก่อน) · กลาง = ลายและเสื้อ / ประวัติ · ขวา = ข้อมูลใบ (รวมหมายเหตุใบ) · ตารางขั้นตอนถอดออก",
  "รอบ 6 (8 ก.ย. ดึก): เบส “ขั้นตอนขอเป็นตาราง · ทุกขั้นของใบนี้เอาออก · จัดแถบนี้ใหม่ดีๆ” → แท็บขั้นตอน = ตารางเดียว แถวละขั้น (ช่อง · ขั้นตอน · สถานะ · ผู้ทำ · ทำแล้ว/ทั้งหมด · ควรเสร็จ · ข้อกำหนดก่อนปิดขั้น) · แถวที่ยืนอยู่ไฮไลต์และติ๊กได้ แถวอื่นติ๊กไม่ได้ · ท้ายตารางบอกว่าเหลือติ๊กกี่ข้อ · จอแคบเป็นการ์ดต่อขั้น",
