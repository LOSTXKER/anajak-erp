...
const OPTIONS = [
  ...
  { value: "rail", label: "R1 · รางแนวตั้ง" },
  { value: "flowmap", label: "R2 · ผังแนวนอน" },
  { value: "twolane", label: "R3 · สายพานคู่" },
  { value: "twolanefull", label: "R3+ · สายพานคู่ เต็มหน้า" },
] as const;
...
> = {
  ...
  rail: {
    name: "R1 · รางแนวตั้ง (เหมือนแผนที่รถไฟ)",
    idea: "ฝั่งซ้ายเปลี่ยนจากตารางเป็นราง",
    summary:
      "รางเส้นเดียวไล่จากบนลงล่างตามลำดับงานจริง · ช่วงไหนที่เดินพร้อมกันได้ การ์ดจะแตกออกข้าง ๆ กันพร้อมป้าย “เดินพร้อมกันได้ 4 ขั้น” · จุดบนรางเป็นไฟบอกสถานะ (เขียว = ผ่านแล้ว · ส้มมีวงแหวน = ขบวนอยู่ตรงนี้ · น้ำเงิน = พร้อมเข้า · เทา = ยังไม่ถึง) และช่วงรางที่ผ่านแล้วเปลี่ยนเป็นสีเขียว · แผงลงมือยังอยู่ทางขวาเหมือนแบบ B",
    tradeoff:
      "แนวตั้งกินความสูง — งาน 9 ขั้นยาวกว่าตารางเดิม ต้องเลื่อนดู · รูปร่างรางเปลี่ยนตามสูตร ถ้าวันหนึ่งขั้นเยอะกว่านี้จะยาวขึ้นอีก",
  },
  flowmap: {
    name: "R2 · ผังแนวนอน (ไล่ซ้ายไปขวา)",
    idea: "อ่านเป็นทางเดินของงาน ไม่ใช่รายการ",
    summary:
      "แต่ละ “ช่วง” เป็นคอลัมน์เรียงซ้ายไปขวา มีลูกศรบนรางเชื่อม (สีเขียวเมื่อช่วงก่อนหน้าจบแล้ว) · ขั้นที่เดินพร้อมกันอยู่ในคอลัมน์เดียวกันซ้อนกันลงมา · เห็นทั้งเส้นทางในสายตาเดียวโดยไม่ต้องเลื่อนลง",
    tradeoff:
      "ต้องเลื่อนแนวนอนบนจอแคบ (6 ช่วง) · การ์ดแคบลงเหลือ 224px จึงใส่รายละเอียดได้น้อยกว่าแบบราง",
  },
  twolane: {
    name: "R3 · สายพานคู่ (สายเรา / สายร้านนอก)",
    idea: "แยกให้เห็นว่าอะไรอยู่ในมือเรา อะไรอยู่ที่คนอื่น",
    summary:
      "สองรางขนานกัน — แถวบน “สายเรา” (เบิกเสื้อ · รับเสื้อ · ตรวจของกลับ · DTF · รีดร้อน · QC · แพ็ก) แถวล่าง “สายร้านนอก” (สั่งตัดเย็บ · งานร้านนอก) · ตรงกลางมีกล่องบอกจุดบรรจบว่า “เริ่มรีดไม่ได้จนกว่าของจากร้านจะกลับมาและผ่านตรวจ”",
    tradeoff:
      "อ่านลำดับเวลารวมยากกว่าสองแบบบน เพราะแต่ละแถวมีเวลาของตัวเอง · ถ้าวันหนึ่งย้ายงานร้านนอกมาทำเอง แถวล่างจะว่าง ต้องออกแบบว่าจะซ่อนหรือคงไว้",
  },
  twolanefull: {
    name: "R3+ · สายพานคู่ เต็มทั้งหน้า",
    idea: "เส้นทางคือหน้า ไม่ใช่การ์ดใบหนึ่งในหน้า",
    summary:
      "R3 เดิมมีแค่หัวใบ + ผังในการ์ด + แผงลงมือขวา — แบบนี้ออกแบบครบทั้งหน้าใบสั่งผลิต: แถบปัญหาที่ทำให้งานไม่เดินอยู่บนสุด · ตัวเลขสำคัญ 4 ช่อง (กำหนดส่ง · ปัญหาค้าง · ผ่านแล้ว · จำนวนทั้งใบ) · **ผังกินเต็มความกว้างเป็นแกนของหน้า ไม่มีกรอบการ์ดครอบ** · ที่ทำงานของขั้นที่เลือกกางใต้ผัง (ซ้ายลงมือ ขวาจำนวนแยกสี/ไซซ์) · ของที่ดูนาน ๆ ครั้ง (จำนวนทั้งใบ · ข้อมูลอ้างอิง · ประวัติ) ยุบเป็นแท็บเดียวท้ายหน้า",
    tradeoff:
      "ที่ทำงานอยู่ใต้ผัง ต้องเลื่อนลงนิดหนึ่งเวลาสลับขั้น (R3 เดิมแผงเกาะขวาเห็นตลอด) · หน้ายาวกว่าเดิมเพราะมีของครบจริง ไม่ได้ตัดส่วนไหนทิ้ง",
  },
};
