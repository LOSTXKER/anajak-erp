...
const COPY: Record<Variant, { name: string; idea: string; summary: string; tradeoff: string }> = {
  ...
  three: {
    name: "A · กระดาษเป็นหลัก — ระบบจดเฉพาะจุดที่ “การกดทำงานให้” (4 จุด + ร้านนอก)",
    idea: "จดเฉพาะจุดที่การกดทำงานอย่างอื่นให้ด้วย (แนว Printful / Printavo) ที่เหลือจดบนกระดาษ — ปรับรอบ 2 หลังรีวิว UX",
    summary:
      "เบิกเสื้อจากสต๊อก (1 ครั้ง — กดแล้วตัดสต็อกให้) · พิมพ์ DTF = ผ่านเองจากรอบพิมพ์ (0 ครั้งในใบนี้) · ผล QC ดี/เสีย (1 ฟอร์ม — ใช้สั่งแก้ + ตัวชี้วัด) · แพ็กเสร็จ (1 ครั้ง — ปลดล็อกส่งของ/ออกบิล) · ร้านนอก ส่งไป/รับกลับ (2 ครั้งต่อร้าน) · รีดร้อนจดบนกระดาษ เมื่อ QC บันทึกระบบขึ้น “ถือว่าผ่าน” สีเทา · ใบผลิตมีแถบ “ใบสั่งงานกระดาษ ฉบับ 1” + ช่วงงานที่อนุมานได้ + ปุ่มพิมพ์ใหม่ · กระดาษ: แถวที่จดในระบบไม่มีช่องยอด · ร้านนอกแยกตาราง “เดินคู่ขนาน” · ท้ายใบมีฉบับ/วันพิมพ์ กันกระดาษเก่า",
    tradeoff: "เวลาของขั้นรีดร้อนไม่มีในระบบ → ตัวชี้วัดผลิตเหลือระดับใบ · จอ TV โรงงานเห็นช่วงงานที่อนุมาน ไม่ใช่สถานะที่ใครกด · แก้แบบกลางทางต้องพิมพ์ใบใหม่และตามเก็บใบเก่า (QR ผูกฉบับช่วยเตือน) · หัวหน้าอยากรู้ว่ารีดถึงไหนต้องเดินไปดู หรือจดให้จากเมนู",
  },
  batch: {
...
const NOTES = [
  "ตัวเลข “แตะจอต่อใบ” นับจากปุ่มที่ระบบบังคับจริง: ปัจจุบัน = เริ่ม + ปิดขั้นพร้อมยอด ทุกขั้น (ร้านนอก ส่ง + รับกลับ + ตรวจรับ) · ทาง A = เฉพาะจุดที่การกดทำงานให้ · ไม่นับติ๊กข้อกำหนด (ของจริงตอนนี้ยังไม่บันทึก) · ปิดรอบพิมพ์ DTF ยังกดที่สถานีพิมพ์ 1 ครั้งต่อรอบ แต่รอบหนึ่งรวมหลายใบ จึงไม่นับในใบนี้",
  "รีวิวรอบ 2 (ux-review + impeccable 5 ก.ย.) เปลี่ยนทาง A 2 เรื่องใหญ่: (1) “เบิกเสื้อจากสต๊อก” ย้ายกลับมาจดในระบบ เพราะปุ่มเบิกตัดยอดสต็อกใน Anajak Stock ให้ — เสื้อที่ลูกค้าเอามาเองไม่ต้อง (ตรวจรับที่หน้าออเดอร์อยู่แล้ว) (2) แถวที่จดในระบบบนกระดาษไม่มีช่องยอด/ลงชื่อ — ยอด QC เขียนที่จอที่เดียว ไม่ลอกซ้ำ",
  "ใบสั่งงานกระดาษ “มีอยู่แล้ว” ในระบบ (พิมพ์จากหน้าออเดอร์) — ทุกทางต่อยอดใบนี้ ไม่ได้สร้างใหม่ · ทาง A ย้ายปุ่มพิมพ์มาอยู่ในใบผลิต (แถบบนรายการขั้น) และระบบจำฉบับที่พิมพ์ · ม็อกอัพเปลี่ยนหลังพิมพ์ = แถบเปลี่ยนเป็นเตือน + QR ใบเก่าเตือนตอนสแกน",
  "สวิตช์ “มีร้านนอก”: ใบตัวอย่างมีปักแขน + ป้ายคอ 2 ร้าน — ร้านนอกเป็นจุดที่ทาง A ยังต้องจด เพราะของออกจากโรงงาน · กดสลับเป็น “ทำเองทั้งใบ” จะเห็นว่าทาง A เหลือแตะจอ 3 ครั้ง",
  "หัวหน้า/ช่าง: ช่างไม่มีเมนู “เพิ่มเติม/แก้ให้” ไม่มีปุ่มพิมพ์ใหม่ และไม่มีปุ่ม “กรอกจากกระดาษ” · ขั้นที่จดบนกระดาษ หัวหน้าจดว่าเสร็จได้จากเมนู “เพิ่มเติม” (ไม่บังคับ) ช่างเห็นแค่ “แจ้งปัญหา”",
  "ปัญหา/สถานะติด/คนทำ ขึ้นทุกขั้นไม่ว่าจดที่ไหน — โหมดจดคุมแค่ยอดกับเวลา (ขั้น 1 ในตัวอย่างติดปัญหาเสื้อขาด 60 ตัว ต้องเห็นเสมอ)",
...
const OUT_OF_SCOPE = [
  "โหมดหน้างาน (คิวสถานีของช่างบนจอทัช) ไม่อยู่ในหน้าลองนี้ — ถ้าเคาะ A คิวจะเหลือเฉพาะสถานี เบิกเสื้อ / QC / แพ็ก / ร้านนอก และสถานีรีดร้อนหายไป · สแกน QR ที่สถานี QC ควรเปิดหน้าลงมือ QC ของใบนั้นเลย (1 สแกน 1 กด)",
  "QC เจอเสียแล้วต้องรีดใหม่ (งานแก้) — ยังไม่มีคำตอบทั้งบนกระดาษและจอ ต้องเคาะกับเบสก่อนลงของจริง (ดูคำถามท้ายรายงาน)",
  "ปุ่มทุกปุ่มยังไม่ทำอะไร · ฟอร์มกรอกจากกระดาษ (ทาง B) ยังไม่บันทึก · สถานะ “ถือว่าผ่าน” สีเทายังไม่มีในข้อมูลตัวอย่าง (QC ยังไม่ได้บันทึก)",
  "ยังไม่ตัดสินว่า “จุดที่ต้องจด” เป็นค่าตั้งได้ในสูตรขั้นงาน หรือเป็นกฎตายตัว — เป็นงานฐานข้อมูล ต้องถามเบสก่อนแยกต่างหาก",
] as const;
