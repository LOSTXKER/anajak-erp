 ...
 * เพิ่มเฉพาะสิ่งที่หน้าลองนี้เทียบ: ขั้นไหน "จดในระบบ" / "จดบนกระดาษ" / "ผ่านเอง" และช่างต้องแตะจอกี่ครั้ง
 *
 * รอบ 2 (2026-09-05 หลังรีวิว ux-review + impeccable): เตรียมเสื้อ "เบิกจากสต๊อก" ย้ายเป็นจดในระบบ
 * เพราะปุ่มเบิกตัดยอดสต็อกใน Anajak Stock ให้ (services/garment-pick.ts) = การกดที่ทำงานให้ ไม่ใช่การจด
 */
...
export const RECORD_MODE: Record<string, RecordMode> = {
  s1: "screen", // เตรียมเสื้อ เบิกจากสต๊อก — กดแล้วตัดสต็อกให้ (เสื้อลูกค้าเอามาเอง = ตรวจรับที่หน้าออเดอร์อยู่แล้ว)
...
/** ทำไมจุดนี้ต้องจดในระบบ — ประโยคที่ขึ้นในโซนลงมือ (การกดต้อง "ทำงานให้" ไม่ใช่แค่จด) */
export const WHY_SCREEN: Record<string, string> = {
  s1: "กดเบิกแล้วระบบตัดยอดสต็อกใน Anajak Stock ให้เลย — ไม่ต้องไปตัดเอง",
...
export const PAPER_NOTE = "ขั้นนี้จดบนกระดาษ — เมื่อ QC บันทึกผล ระบบจะขึ้นว่า “ถือว่าผ่าน” (สีเทา ไม่ใช่ “ผ่านแล้ว” สีเขียว)";
export const AUTO_NOTE = "ปิดรอบพิมพ์ PR-2608-014 แล้ว 28 ส.ค. 11:30 — ขั้นนี้ผ่านเองจากรอบพิมพ์ ไม่ต้องกดซ้ำในใบนี้";
...
/** ใบกระดาษที่พิมพ์ไปแล้วของใบผลิตนี้ — ระบบจำฉบับไว้ กันกระดาษเก่าตอนแบบเปลี่ยน */
export const TICKET = { issue: 1, printedAt: "27 ส.ค. 09:12", mockup: "v3", printedBy: "เบส" } as const;

/** ช่วงงานที่ระบบอนุมานได้จากจุดที่จด (ทาง A) — ไม่ใช่สถานะที่ใครกด */
export const INFERRED_STAGE = { now: "หลังพิมพ์ DTF → ก่อน QC", detail: "รีดร้อนดูจากกระดาษ · ปักแขน/ป้ายคอยังไม่กลับ", detailNoOutsource: "รีดร้อนดูจากกระดาษ" } as const;
...
/** ปุ่มที่ระบบบังคับให้คนหน้างานกดในขั้นนี้ (ใช้คำนวณ "แตะจอต่อใบ") */
