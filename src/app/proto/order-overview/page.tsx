...
> = {
  current: {
    name: "ของจริงตอนนี้",
    idea: "ภาพรวม = ที่รวมข้อมูลทุกอย่างที่ไม่ใช่รายการสินค้า",
    summary:
      "component ตัวจริงของหน้า /orders/[id] ต่างจากหน้าจริงแค่ข้อมูลปลอม — ซ้ายเป็นรายละเอียดงาน (ตัวหนังสือล้วน) กับการ์ดลูกค้าที่ยาวที่สุดในหน้า ขวาเป็นข้อมูลออเดอร์ · จัดส่ง · แบรนด์ · บรรทัดเวลา",
    tradeoff:
      "ทั้งแท็บไม่มีรูปลายแม้แต่รูปเดียว — เปิดใบงานมาแล้วยังไม่รู้ว่างานนี้พิมพ์อะไร ต้องกดไปแท็บ “ม็อกอัพ & ไฟล์” ทุกครั้ง · การ์ดลูกค้ากินคอลัมน์ซ้ายเกือบทั้งแถบด้วยข้อมูลที่นาน ๆ ใช้ (ที่อยู่ออกบิล · ป้าย · ประวัติ 4 ช่อง)",
  },
  strip: {
    name: "A · แถบลายบนสุด",
    idea: "เติมของที่ขาด ไม่รื้ออะไรเลย",
    summary:
      "เพิ่มการ์ด “ลายและไฟล์งาน” เต็มความกว้างไว้บนสุด — ป้ายสถานะแบบ · รูปม็อกอัพทุกด้าน (ตัวดูรูปตัวจริงของระบบ) · จำนวนไฟล์ลูกค้า/ไฟล์พิมพ์ · ปุ่มไปแท็บม็อกอัพ ส่วนข้างล่างคือของเดิมทั้งดุ้น ไม่ขยับสักช่อง",
    tradeoff:
      "ตอบโจทย์ “เห็นลายทันที” แต่ **ไม่ได้ทำให้กระชับขึ้นเลย** — หน้ายาวกว่าเดิม เพราะรูปไปเพิ่มบนของเดิมที่ยังยาวเท่าเดิม · บนมือถือต้องเลื่อนผ่านรูปก่อนถึงกำหนดส่ง/ยอด",
  },
  pair: {
    name: "B · ลายคู่ข้อมูล",
    idea: "ภาพรวมตอบ 3 คำถาม — ทำอะไร · ให้ใคร · ส่งเมื่อไหร่เท่าไร",
    summary:
      "ซ้ายบนสุดเป็นการ์ด “งานนี้พิมพ์อะไร”: รูปใหญ่ด้านหน้า + ด้านที่เหลือเรียงข้าง + รายละเอียดงาน + รายชื่อไฟล์ (ไฟล์พิมพ์ติดกุญแจ) รวมอยู่ในการ์ดเดียว เพราะพูดเรื่องเดียวกัน · การ์ดลูกค้าย่อลง: ประวัติ 4 ช่อง (ยอดสะสม · จำนวนครั้ง · สั่งล่าสุด · วงเงิน) ยุบเป็นบรรทัดเดียวใต้ชื่อ ค่าอยู่ครบไม่ได้ตัดทิ้ง · คอลัมน์ขวาเหมือนของจริงทุกช่อง",
    tradeoff:
      "รูปใหญ่ 1 ใบ = เห็นด้านหน้าชัด แต่ด้านหลัง/แขนเล็กลงเหลือรูปข้าง ๆ · ประวัติลูกค้าเป็นบรรทัดเดียวอ่านเร็วก็จริง แต่เทียบตัวเลขยากกว่าตอนเป็นช่องแยก · ยังเป็นสองคอลัมน์เหมือนเดิม คนที่บอกว่า “ข้อมูลเยอะไป” จะยังรู้สึกเยอะอยู่",
  },
  cover: {
    name: "C · ลายคือหน้าปก",
    idea: "ภาพรวมคือหน้าปกของงาน ไม่ใช่แฟ้มข้อมูล",
    summary:
      "เรียงตามลำดับที่คนถามจริง — ① แถบข้อเท็จจริงบนสุดไม่มีการ์ด (กำหนดส่ง · จำนวน · ยอด · ลูกค้า · สถานะแบบ) ② ลายเต็มแถวบนพื้นจม พร้อมรายละเอียดงานกับไฟล์อยู่ขวามือ ③ ข้อมูลที่เหลือยุบเป็นแถวป้าย-ค่าตัวเล็กสามคอลัมน์ — ทุกช่องที่ของจริงมียังอยู่ครบ ไม่มีอันไหนหาย",
    tradeoff:
      "กระชับที่สุดในสามแบบ แต่แถวเล็กเรียงถี่ = ตาทำงานหนักกว่าเมื่อต้องอ่านนาน ๆ · ค่าไทยยาว (ที่อยู่ · หมายเหตุ) ทำให้แถวสูงไม่เท่ากันจนคอลัมน์ดูไม่เรียบ · บนมือถือสามคอลัมน์ยุบเป็นคอลัมน์เดียว ความกระชับที่ได้บนคอมจึงหายไปเกือบหมด",
  },
};
...
const HEIGHTS = [
  { value: "current", name: "ของจริงตอนนี้", desktop: "1,605 px", mobile: "3,677 px" },
  { value: "strip", name: "A · แถบลายบนสุด", desktop: "2,087 px (+30%)", mobile: "4,232 px (+15%)" },
  { value: "pair", name: "B · ลายคู่ข้อมูล", desktop: "1,950 px (+21%)", mobile: "3,779 px (+3%)" },
  { value: "cover", name: "C · ลายคือหน้าปก", desktop: "1,666 px (+4%)", mobile: "3,699 px (+1%)" },
] as const;
...
const OUT_OF_SCOPE = [
  ...
  "รูปในหน้าลองเป็นภาพวาดปลอมที่ทำขึ้นให้ตรงกับใบตัวอย่าง (โปโลกรมท่า ปักอกซ้าย + DTF หลัง) — ของจริงเป็นรูปถ่าย/ไฟล์กราฟิกจาก R2 ซึ่งสัดส่วนไม่เท่ากันทุกใบ ทุกแบบจึงบังคับกรอบสี่เหลี่ยมจัตุรัสเหมือนของจริง",
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <section className="mt-8 card-surface rounded-2xl p-5">
          ...
          <p className="mt-1 text-xs text-muted">
            ความสูงทั้งหน้าของใบตัวอย่างใบเดียวกัน วัดด้วยเบราว์เซอร์เมื่อ 31 ส.ค. 2569 —
            ยิ่งน้อยยิ่งต้องเลื่อนน้อย
          </p>
