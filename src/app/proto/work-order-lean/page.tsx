...
const COPY: Record<Variant, { name: string; idea: string; summary: string; tradeoff: string }> = {
  ...
  one: {
    ...
    summary:
      "การ์ดเดียวจากบนลงล่าง: หัวใบ (เลขที่ · ลูกค้า · กำหนดส่ง · จำนวน · พิมพ์) → แผนที่เส้นทาง — ขั้นที่กดสว่างขึ้นเหมือนแท็บ → (ใบที่มีหลายสาย: แถวชิป “ตอนนี้ 4 สาย” กดสลับ) → บรรทัดเสื้อ → เนื้อของขั้นที่เลือกต่อลงมาเลย ไม่มีหัวข้อ “ตอนนี้ทำได้” ไม่มีกรอบซ้อน → ท้ายการ์ดเป็นลิงก์ “ลายและม็อกอัพ · ข้อมูลใบ · ประวัติ” กระโดดไปกล่องพับข้างล่าง · เปิดมาเลือกขั้นที่เร่งสุดให้ (ติดปัญหา → ของอยู่ที่ร้าน → กำลังทำ)",
    tradeoff:
      "เห็นทีละขั้น — ใบซับซ้อนที่มี 4 สายพร้อมกัน ต้องกด 3 ครั้งถึงเห็นครบ (E โชว์ทุกใบพร้อมกัน) · การ์ดยาวขึ้นบนมือถือ (แผนที่ + เสื้อ + ขั้น อยู่ในกรอบเดียวเลื่อนยาว) · แผนที่กับเนื้อขั้นผูกกันด้วยสี ถ้ากดขั้นที่ปิดแล้วเนื้อจะเป็นแค่ “ปิดขั้นแล้ว” บรรทัดเดียว",
  },
  head: {
...
const NOTES = [
  ...
  "“กล่องที่เห็นทันที” นับกล่อง/การ์ดที่มีขอบของตัวเองก่อนกดอะไร รวมกล่องพับ 3 กล่อง · “ข้อมูลซ้ำ” นับจากใบหลัก: 30 ตัว ×3 · สถานะขั้น ×3 · ร้านนอก ×3 · ผ่าน/ติดปัญหา ×2 = ซ้ำ 8 ที่",
...
const OUT_OF_SCOPE = [
  ...
  "การ์ดเบิกเสื้อ (GarmentPickCard) · รายการวัตถุดิบ (MaterialUsage) · ม็อกอัพทุกเวอร์ชัน — ของจริงมี หน้าลองใส่เป็นบรรทัดบอกไว้ ไม่จำลอง",
    ...
    <main className="min-h-screen bg-surface-muted px-4 py-8 text-strong sm:px-6 lg:px-10">
      <div className="mx-auto max-w-[1500px]">
        ...
        <p className="mt-2 max-w-4xl text-sm text-secondary">
          เบสเปิดใบ ORD-2609-0009 (7 ก.ย.) หลังลงแบบ E ไปเมื่อวาน แล้วบอก “ยังดูเยอะไปอยู่ ช่วยรื้อจัดใหม่” · คำถามที่ต้องเคาะ —{" "}
          <span className="font-medium text-strong">จะเบาลงด้วยการตัดของซ้ำบนโครงเดิม (A) · รวมทุกอย่างเป็นการ์ดเดียวเห็นทีละขั้น (B) · หรือให้ประโยคเดียวนำก่อนแล้วค่อยเห็นโครง (C)</span>
        </p>
