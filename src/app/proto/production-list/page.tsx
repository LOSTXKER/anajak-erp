...
> = {
  current: {
    ...
    summary:
      "การ์ดตัวเลข 5 ใบสูง 87px (บนมือถือกลายเป็น 3 แถว) · ตารางหกคอลัมน์ โดยคอลัมน์กลางคือ “ต้องทำต่อ” ซึ่งเขียนทั้งเหตุผล เจ้าของถัดไป และป้ายสายงาน · แถวสูงจริง 107px (โค้ดตั้งไว้ 82px แต่เนื้อในสามบรรทัดดันให้สูงกว่านั้น) จึงเห็นงานได้ 5 แถวต่อหนึ่งจอ",
    tradeoff:
      "เป็นแบบเดียวที่ยังบอกว่า “ต้องทำต่ออะไร” ซึ่งเบสสั่งแล้วว่าไม่ต้อง — เก็บไว้ในหน้าลองเพื่อให้เห็นว่าตัดออกแล้วต่างแค่ไหน · ต้องเลื่อน 328px กว่าจะถึงงานใบแรก · การ์ดที่เลือกอยู่บอกด้วยสีเส้นขอบอย่างเดียว ซึ่งแทบมองไม่เห็น",
  },
  dense: {
    name: "A · แน่น",
    idea: "หน้านี้คือรายการงาน ไม่ใช่แดชบอร์ด",
    summary:
      "การ์ดกรองบีบจาก 87px เหลือ 56px (ไอคอน–ชื่อ–ตัวเลข อยู่บรรทัดเดียว) · คอลัมน์กลางเป็น “สถานะ”: ชื่อสถานะกับจุดสี พร้อมบรรทัดเล็กบอกสายงานที่ค้างอยู่ เช่น “กำลังผลิต / เตรียมเสื้อ · DTF” · แถวลดจาก 107px เหลือ 61px · ผลคือเห็น 9 แถวต่อจอ แทนที่จะเป็น 5 และทั้งหน้าสั้นลงจาก 1,676px เหลือ 1,104px",
    tradeoff:
      "ได้จำนวนงานต่อจอเกือบเท่าตัว แต่ตัวเลขบนการ์ดเล็กลงจนไม่ทำหน้าที่ “ป้ายบอกสถานการณ์” อีกต่อไป · ชื่อบริษัทยาว ๆ โดนตัดคำมากกว่าเดิม · การ์ดกรองเตี้ยลงเหลือสูง 56px ซึ่งเป็นข้อเสียบนจอทัชในโรงงานที่นิ้วต้องกดโดน",
  },
  focus: {
    name: "B · ชัด",
    idea: "ขนาดใกล้เดิม แต่ให้ความเร่งด่วนอ่านได้จากหางตา",
    summary:
      "การ์ดที่เลือกอยู่ย้อมพื้นทั้งใบ + มีขีดสีบนหัว (ไม่ใช่แค่เส้นขอบ) และเพิ่มบรรทัดขยายว่ากองนั้น “เลยกำหนดกี่ใบ” · ในตาราง แถวมีแถบสีทางซ้าย (แดง = มีขั้นงานพัง · เหลือง = ติดรอของ) · คอลัมน์สถานะเหลือชื่อสถานะกับจุดสีล้วน เพราะช่องถัดไปเป็น “รางช่วงงาน” 6 จุด (เตรียมเสื้อ · พิมพ์ฟิล์ม · รีดร้อน · ร้านนอก · ตรวจ QC · แพ็ค) พร้อมบรรทัด “อยู่ที่ …” ซึ่งบอกอยู่แล้วว่าค้างช่วงไหน · แถว 85px เห็น 6 แถวต่อจอ ทั้งหน้า 1,405px",
    tradeoff:
      "การ์ดสูงขึ้น 19px เพราะมีบรรทัดขยาย จึงต้องเลื่อนไกลกว่าเดิมนิดหน่อยกว่าจะถึงงานใบแรก (347px เทียบกับ 328px) · แน่นสู้แบบ A ไม่ได้ — ได้ 6 แถวต่อจอ ไม่ใช่ 9 · วันที่งานแดงเยอะ ๆ แถบสีซ้ายจะกลายเป็นแถบแดงยาวจนไม่ช่วยแยกอะไร · จุดบนรางเล็ก ต้องเอาเมาส์ชี้ถึงจะรู้ชื่อช่วง (บนมือถือชี้ไม่ได้ จึงเหลือแค่บรรทัด “อยู่ที่ …”)",
  },
  bar: {
    name: "C · แถบเดียว",
    idea: "เอาการ์ดออก แล้วให้รายการบอกเวลาแทน",
    summary:
      "ตัวกรอง 5 มุมยุบเป็นแถบเดียวสูง 36px ในแถบเครื่องมือ (ใช้ปุ่มกรองตัวจริงของระบบ มีตัวเลขติดในชิป) — ไม่มีการ์ดเลย · ตารางแบ่งหัวข้อตามกำหนดส่ง: เลยกำหนดแล้ว · ส่งวันนี้ · ส่งพรุ่งนี้ · ภายในสัปดาห์นี้ · หลังจากนั้น · ยังไม่กำหนดส่ง · คอลัมน์สถานะเหมือนแบบ A · คอลัมน์จำนวนโผล่ตั้งแต่จอกลาง (ของเดิมต้องจอกว้างมาก) และในแถวเหลือแค่วันที่ เพราะหัวข้อกลุ่มบอกความเร่งแล้ว · แถว 82px เห็น 6 แถวต่อจอ และงานใบแรกมาถึงที่ 299px ซึ่งเร็วที่สุดในสี่แบบ",
    tradeoff:
      "เสียตัวเลขสรุปตัวใหญ่ไปเลย (เหลือเลขจิ๋วในชิป) — ใครที่ชินกับการกวาดตาดู “ต้องจัดการ 4” ตอนเปิดหน้า จะไม่มีอะไรให้กวาดอีก · การจัดกลุ่มทับการเรียง — กด “เรียงตามจำนวน” ที่หัวตารางจะเรียงภายในกลุ่มเท่านั้น ไม่ใช่ทั้งตาราง · ได้พื้นที่คืนน้อยกว่าที่คิด เพราะแถบตัวกรอง 36px มาแทนการ์ด 87px งานใบแรกจึงมาถึงเร็วขึ้นแค่ 29px · งานเยอะ ๆ จะมีหัวข้อกลุ่มคั่นถี่",
  },
};

/** ข้อเท็จจริงที่วัดมาจากของจริง ไม่ใช่ความเห็น */
const NOTES = [
  "ทั้งสี่แบบอ่านจากข้อมูลก้อนเดียวกัน ที่เดินผ่าน buildProductionBoard() ตัวจริง — ชื่อสถานะยกมาจาก INTERNAL_STATUS_LABELS ของระบบ · ป้ายสถานะใช้ StatusLabel ตัวกลางของเว็บ (จุดสี + ข้อความ แบบเดียวกับทะเบียนออเดอร์) · ตัวเลขบนการ์ดกับลำดับการเรียงมาจากสูตรเดียวกับหน้าจริง ไม่มีข้อความไหนแต่งขึ้นเอง",
  "สีของจุดหน้าสถานะบอก “สภาพงาน” ไม่ใช่ “ต้องทำอะไร” — แดง = มีขั้นงานพัง · เหลือง = ติดรอของ/รอมัดจำ · น้ำเงิน = เดินอยู่ · เขียว = พร้อมส่ง · เทา = ยังไม่เริ่ม · ส่วน “เลยกำหนด / ส่งวันนี้” ยังอยู่ที่คอลัมน์กำหนดส่งเหมือนเดิม",
  ...
  "มีครบทุกสถานะขอบที่หน้าจริงเจอ: เลยกำหนด · ส่งวันนี้ · ด่วน · ไม่กำหนดส่ง · ขั้นงานพัง · รีดร้อนติดรอของร้านนอก · คิวติดด่านมัดจำ · งานผสมสามสาย · ชื่อบริษัทยาวจนต้องตัดคำ · ออเดอร์ที่ยังไม่มีม็อกอัพ (ขึ้นกรอบประว่าง)",
...
const OUT_OF_SCOPE = [
  ...
  "ช่อง “ของจริงตอนนี้” ยังมีคอลัมน์ “ต้องทำต่อ” อยู่ เพราะเป็น component ตัวจริงที่ยังไม่ได้แก้ — จงใจเก็บไว้ให้เทียบกับสามแบบที่ตัดออกแล้ว",
    ...
    <main className="min-h-screen bg-surface-muted px-4 py-8 text-strong sm:px-6 lg:px-10">
      <div className="mx-auto max-w-[1500px]">
        ...
        <p className="mt-2 max-w-4xl text-sm text-secondary">
          โจทย์คือ “ทำหน้านี้ใหม่ เฉพาะการ์ดกรองกับตาราง” และเบสสั่งเพิ่มว่า
          <strong className="font-semibold text-strong">ตารางไม่ต้องบอกว่าต้องทำต่ออะไร ขอแค่รู้ว่าตอนนี้สถานะอะไรก็พอ</strong> —
          ทั้งสามแบบจึงตัดคอลัมน์ “ต้องทำต่อ” (เหตุผล + เจ้าของถัดไป) ออกแล้ว เหลือคอลัมน์ “สถานะ”
          ส่วนที่ต่างกันคือคิดคนละอย่างว่า “ใหม่” ควรแปลว่าอะไร: เห็นงานเยอะขึ้น ·
          เห็นความเร่งด่วนชัดขึ้น · หรือเอาการ์ดออกไปเลยแล้วให้รายการบอกเวลาแทน
        ...
        <section className="card-surface mt-8 rounded-2xl p-5">
          ...
          <p className="mt-1 text-xs text-muted">
            ...
            แบบ B/C แถวเตี้ยกว่าเดิมเพราะย้ายเจ้าของถัดไปกับป้ายสายงานมาอยู่บรรทัดเดียวกัน ·
            แบบ C มีหัวข้อกลุ่มมาคั่นเพิ่มอีก 6 แถบ
          </p>
