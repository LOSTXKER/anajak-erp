...
## ตอนนี้ (2026-09-03)
- **ลงของจริงจอสถานี `/station` แบบ A แล้ว (เบส "งั้นเอา A")** — branch `proto/station-2026-09-03` ยังไม่ merge: `lib/station-desk.ts` (+9 test) · `components/station/` (screen/pieces/job) · `factory.stationQueue` เพิ่มรูปปก/ร้านนอก/รอบพิมพ์ · ใบผลิต D แยก `work-order-controller`/`work-order-pieces` (จอสถานีใช้ปุ่ม+dialog ชุดเดียวกัน) · ปุ่ม "จอสถานี" ที่หัวหน้าการผลิต · ด่าน typecheck/lint/test 1622/verify:ui+ratchet ผ่าน · เปิดดูเอง (HTML นิ่งข้อมูลปลอม 1024 ทั้ง 2 บทบาท 2 ธีม) — **ยังไม่เห็นกับข้อมูลจริง/ยังไม่ได้กด mutation จริง (ต้องล็อกอิน)** · หนี้: QC/แพ็กยังพาไปหน้าออเดอร์ · ย้อนขั้นที่ปิดแล้ว server ไม่รับ · ติ๊กข้อกำหนดไม่บันทึก (ROADMAP §A2)
- **หน้าลองจอสถานี `/proto/station` (branch `proto/station-2026-09-03` · เบสตั้งโจทย์ 09-03: "พนักงานไม่เก่งคอม ดูแล้วรู้ว่าต้องทำอะไร · หัวหน้าจัดการได้แต่ละสถานี ช่างกดมั่วก็แก้ให้ได้ · ต่อเติมได้")** — 3 ทาง (A หยิบงานเอง / B หัวหน้าจ่ายงาน / C ใบผลิตเป็นศูนย์กลาง) + สรุปของที่ถอดไป · สวิตช์ พนักงาน/หัวหน้า · คิวว่าง · ธีม · กดเล่นได้จริง (เลือกสถานี → คิว → ลงมือ · ติ๊กข้อกำหนด · แป้นยอด · แจ้งปัญหา · ย้อนกลับ · dialog "แก้ให้") · ข้อมูล 15 ใบ 7 สถานี 5 คน ใน `_data.ts` · ผ่าน typecheck/lint/verify:ui+ratchet · เปิดดูเองแล้ว (HTML นิ่ง 1024×768 ทั้ง 3 ทาง × 2 บทบาท × 2 ธีม — dialog ยังไม่ได้เห็นเพราะ Radix ไม่เรนเดอร์นิ่ง) · **รอเบสเคาะ** (ROADMAP §A2) · ไม่แตะของจริง
- **ลงของจริงใบผลิต `/production/[id]` แบบ D แล้ว (เบส "เอาแบบนี้เลย")**: `src/components/production/work-order-page.tsx` + `src/lib/work-order-standards.ts` (ข้อกำหนดต่อขั้น v1 นิ่ง) · หัวใบตัวเลข 4 ช่อง + การ์ดปัญหา → แท็บ ขั้นงาน/ทำอะไร/ข้อมูลใบ/ประวัติ ทุกแท็บ 2 คอลัมน์ · ปุ่มลงมือจาก `selectNowSteps` + `evaluateHeatPressGate` เดิม · dialog/การ์ดเดิม 7 ไฟล์คืนจาก git (step-update/outsource/qty · garment-pick · design-card · mockup-tab · material-usage · station-garment-preview) · ด่าน typecheck/lint/test/verify:ui/ratchet ผ่าน · **ยังไม่เห็นกับข้อมูลจริง (ต้องล็อกอิน) — เบสเปิด `/production/<id>` ดู** · ยังไม่ merge main
...
## NEXT
0. **เบสเปิด `localhost:3000/station` (branch `proto/station-2026-09-03` · ล็อกอินบัญชีตัวเอง = โหมดหัวหน้า) กดทีละชั้น เลือกสถานี → คิว → ลงมือ → แก้ให้** แล้วบอกจุดที่ไม่ตรงหน้างาน → แก้ → build → merge main (เบสสั่ง) · ต่อไป: หน้าลงมือ QC/แพ็ก บนจอนี้ (§A2)
1. **เบสเปิดใบผลิตของจริงบน dev ดูกับข้อมูลจริง** (กดจากหน้าการผลิต) → บอกจุดที่ไม่ตรง → แก้ → build → merge main (เบสสั่ง) · หนี้แบบ D อยู่ ROADMAP §A (checklist ติ๊กได้ต้องเพิ่ม schema — ถามก่อน)
