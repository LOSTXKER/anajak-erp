...
## ตอนนี้ (2026-09-03)
- **หน้าลอง `/proto/desk-station` — "ใบผลิตหลังแบ่งตามที่ยืน" (branch `proto/desk-station-2026-09-03` · ROADMAP §A4) — รอเบสเคาะ A/B**: เบสถามว่าใบผลิตกับจอสถานีซ้ำกันไหม → ตรวจแล้วปุ่มลงมือชุดเดียวกันอยู่ 2 หน้า + จอสถานีไม่อยู่ในเมนู → เสนอ 3 ทาง เบสเคาะ **"เอา A · แบ่งตามที่ยืน"** (จอสถานี = ลงมือ · ใบผลิต = ดู+วางแผน ไม่มีปุ่มลงมือ) → หน้าลองถามต่อว่าแท็บขั้นงานควรหน้าตาแบบไหน: ปัจจุบัน (D) / **A คงโครง D แทนโซนลงมือด้วย "การ์ดที่ยืน"** (งานอยู่ไหนใครถือ → สถานี/คน/ควรเสร็จ → ปุ่มวางแผนหัวหน้า + "ไปทำที่จอสถานี") / **B ตารางแผนทั้งใบ** (6 คอลัมน์ กดแถวกางการ์ดที่ยืนย่อ) · สวิตช์หัวหน้า/ช่าง · ธีม · ตารางโครง ใคร-ยืนไหน-หน้าไหน-ทำอะไรได้ บนหน้า · ใช้ใบตัวอย่าง+แท็บอื่นของ `/proto/work-order` (export คอลัมน์จาก `tabs.tsx`) · ด่าน typecheck/lint/test 1622/verify:ui ผ่าน · เปิดดูเอง (HTML นิ่ง 1280 A/B × หัวหน้า/ช่าง × สว่าง/มืด + มือถือ 390) · ไม่แตะของจริง
- **จอสถานี `/station` แบบ A ขึ้นเว็บจริงแล้ว (09-03 · main `5011ed6..bd38e3f` · build ผ่าน)**: `lib/station-desk.ts` (+9 test) · `components/station/` (screen/pieces/job) · `factory.stationQueue` เพิ่มรูปปก/ร้านนอก/รอบพิมพ์ · ใบผลิต D แยก `work-order-controller`/`work-order-pieces` (จอสถานีใช้ปุ่ม+dialog ชุดเดียวกัน) · ปุ่ม "จอสถานี" ที่หัวหน้าการผลิต — **ยังไม่เห็นกับข้อมูลจริง/ยังไม่ได้กด mutation จริง (ต้องล็อกอิน)** · หนี้: QC/แพ็กยังพาไปหน้าออเดอร์ · ย้อนขั้นที่ปิดแล้ว server ไม่รับ · ติ๊กข้อกำหนดไม่บันทึก · DTF_PRINT ไม่มีปุ่ม (ROADMAP §A2)
- หน้าลองจอสถานี `/proto/station` (3 ทาง A/B/C) — เคาะแล้ว A · ทะเบียน `เคาะแล้ว`
- **ใบผลิต `/production/[id]` แบบ D ขึ้นเว็บจริงแล้ว** (`work-order-page.tsx` + `lib/work-order-standards.ts`) · หัวใบ 4 ช่อง + การ์ดปัญหา → แท็บ 4 แท็บ 2 คอลัมน์ · dialog เดิม 7 ไฟล์คืนจาก git · **ยังไม่เห็นกับข้อมูลจริง (ต้องล็อกอิน)** · หนี้แบบ D → ROADMAP §A · **จะถูกถอดปุ่มลงมือออกตาม §A4 หลังเบสเคาะหน้าลอง**
- หน้าลองใบผลิต `/proto/work-order` — เคาะแล้ว D · ยังเปิดได้ (หน้าลอง desk-station ยืมข้อมูล/คอลัมน์จากที่นี่)
- ratchet ข้ามไฟล์คำอธิบายของหน้าลอง (`proto/**/page.tsx` · `_variants/removed.tsx`) · baseline dots 43 · muted 297
...
## NEXT
0. **เบสเปิด `localhost:3000/proto/desk-station` (branch `proto/desk-station-2026-09-03`) แล้วตอบ A หรือ B** → ลงของจริง §A4 (ถอดโซนลงมือจากใบผลิต → การ์ดที่ยืน + ลิงก์ไปจอสถานี · เมนูซ้าย "จอสถานี" · ช่างล็อกอินตกที่ /station · DESIGN.md) → build → merge main (เบสสั่ง)
1. **เบสเปิด `/station` บนเว็บจริงด้วยบัญชีตัวเอง กดกับใบจริง 1-2 ใบ (แก้ให้ · แจ้งปัญหา)** แล้วบอกจุดที่ไม่ตรง · ต่อไป: หน้าลงมือ QC/แพ็ก บนจอนี้ (§A2)
2. เบสเปิดใบผลิตของจริงดูกับข้อมูลจริง → บอกจุดที่ไม่ตรง (หนี้แบบ D → ROADMAP §A · checklist ติ๊กได้ต้องเพิ่ม schema — ถามก่อน)
3. หลังจากนั้น: ทางเข้ารอบพิมพ์/คลังฟิล์ม · มือถือหน้าการผลิตเป็นการ์ด (§A3) · ซ้อม V2 ต่อเมื่อจอสถานีใช้ได้ (§B) · ว่างค่อย MFG1-3 (§C)
