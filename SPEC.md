...
- [x] **ใบผลิตเป็น Job Jacket ทีละขั้น** — มีทางกลับคิวเพียงจุดเดียวและไม่วาด breadcrumb/local module nav ที่ชวนออกจากใบงาน · ERP ใช้แถบตัวตนงานแบบกระชับ → route ribbon จาก workflow จริง → operation canvas ของขั้นที่เลือกเพียงขั้นเดียว ซึ่งตอบว่า **ต้องทำอะไร / ติดอะไร / ใช้แบบและจำนวนไหน / กดปุ่มใด** โดย primary action มีจุดเดียว · การเลือกขั้นเป็น view state ไม่เปลี่ยน workflow และต้องคงหลาย current สำหรับงานหลายเลน · เสื้อที่เป็นงานปัจจุบันหรือใบเก่าที่ระบบยืนยันยอดไม่ได้อยู่ติด action และต้องยืนยันก่อนเริ่มรีด · เสื้อ/วัตถุดิบระดับทั้งใบกับเส้นทางทั้งหมดเปิดใน inspector ตามต้องการ; deep link เดิมต้องเข้าหัวข้อถูกและไม่สร้าง mutation ซ้ำ · Station variant คงหน้าเส้นตรงเฉพาะสถานีปัจจุบัน ไม่ mount Job Jacket/inspector/MaterialUsage หรือข้อมูลเงิน
