...
- [x] **`/production` แบบ A “โต๊ะงานหัวหน้า”** (เบสเคาะ 09-02): ตัวเลขใหญ่ 4 ช่องเป็นตัวกรอง · ตาราง 8 คอลัมน์ กองตามความรีบ · ไม่มีปุ่มในแถว กดทั้งแถวเปิดใบผลิต · คอลัมน์ผู้รับผิดชอบ · ปุ่มเปิดใบผลิตกรองไปกองรอเปิดใบ
- [x] **โมดูลผลิต = หนึ่งโมดูล สองสายตา (เบสเคาะ 09-03 “เอาเลย” · แบ่งตามบทบาท ไม่ใช่สถานที่ — ตรงแนว Odoo Shop Floor / SAP POD)**: หัวหน้า/เบส รู้จักแค่ “การผลิต” — `/production` โต๊ะงาน → `/production/[id]` ใบผลิต**ทำได้ครบในหน้าเดียว** (ดู · วางแผน · ลงมือ · “แก้ให้” · แจ้งปัญหากดเลือกเหตุ) ทุกขั้นบอกว่าอยู่สถานีไหน · ช่าง (`PRODUCTION_STAFF` ไม่มีสิทธิ์หัวหน้า) รู้จักแค่ “งานของฉัน” — ล็อกอิน/เปิดการผลิต/เปิดใบ ถูกพาไป**โหมดหน้างาน** `/production/floor` (คิวสถานี → หน้าลงมือ ปุ่มใหญ่ ไม่มีเงิน) · หัวหน้าเดินโรงงานเปิดจอเดียวกันเป็นแผงสถานี + แก้ให้ · ปุ่ม/dialog/สิทธิ์ชุดเดียว (`work-order-controller` + `step-command-dialogs`) · **ห้ามถอดปุ่มลงมือออกจากใบผลิตอีก** (เบสท้วง 09-03 “หัวหน้าต้องดูจอ 2 ที่ ยุ่งยาก”) · **หน้าตาใบผลิต = แบบ E “ตอนนี้ทำอะไร (รู้ทางขนาน)” (เบสเคาะ 09-06 หลังบอกว่าแบบ D ยังใช้ยาก)**: แผนที่เส้นทางสายขนาน + การ์ด “ตอนนี้ทำได้” สายละใบ · ส่วนที่เคยเป็นแท็บพับไว้ท้าย · “รอ X + Y” มาจาก `lib/work-order-route` ซึ่งวาดกติกาเดิม (รีดร้อน = HeatPressGate · หางงาน = stepsBlockingQc) ไม่ใช่กติกาสถานะใหม่
- [x] **กระดาษเป็นหลัก — ระบบจดเฉพาะจุดที่การกดทำงานให้ (เบสเคาะ A 2026-09-05 · "ทำเลย" 09-06 · ค้นคว้า Printful/Printify/Printavo ใน bestos records)**: โหมดจดต่อขั้นจาก `lib/work-order-record-mode.ts` — **จดในระบบ** = เบิกเสื้อ (ตัดยอด Anajak Stock) · ตรวจรับเสื้อลูกค้า · ร้านนอกทุกทาง (ของออกจากโรงงาน) · **ผ่านเอง** = พิมพ์ DTF (ปิดรอบพิมพ์) · **จดบนกระดาษ** = ที่เหลือ (รีดร้อน ฯลฯ) ช่างติ๊กข้อกำหนด/เขียนยอด/ลงชื่อบนใบสั่งงาน ไม่มีปุ่มหลักบนจอ · จุดถัดไปที่จดคือ QC: `production.sendToQc` ปิดขั้นกระดาษเป็น **"ถือว่าผ่าน"** (สีเทา แยกจาก "ผ่านแล้ว" เขียว · marker ใน notes) แล้ว finalize ใบ — ขั้นที่จดในระบบหรือติดปัญหา/พักยังกั้น · หัวหน้าจดให้เองก่อนได้จากเมนู "เพิ่มเติม" · **กระดาษกับจอห้ามถือข้อมูลเดียวกันสองที่**: ใบสั่งงานพิมพ์จากใบผลิต (`/print/job-ticket/[orderId]?production=`) แถวที่จดในระบบไม่มีช่องยอด/เสร็จ/ลงชื่อ · ร้านนอกแยกตาราง "เดินคู่ขนาน" · ท้ายใบมีวันพิมพ์ + ม็อกอัพเวอร์ชัน และ QR พก `?mockup=n` — ใบผลิตขึ้น Alert แดงเมื่อสแกนใบเก่า · ใบผลิตมีแถบ "ใบสั่งงานกระดาษ" + "ตอนนี้: หลัง X → ก่อน Y" (ช่วงงานอนุมาน ไม่ใช่สถานะที่ใครกด) · ด่าน `verify-work-order-ui` ครอบ ขั้นกระดาษไม่มีปุ่มหลัก / ขั้นจดในระบบมีปุ่มหลัก 1 + เหตุผล / ถือว่าผ่าน
- [x] กติกาเดิมของ `/production` ที่ยังใช้: ตัวกรองเป็นสายพานขั้นงาน (ตัวเลขนำ ชื่อรอง + ลูกศร + ไอคอนรถบนขั้นร้านนอก · เก็บใน URL `?station=`) · ไม่มีช่องเรียงบนคอม · ตารางแบ่งกลุ่มตามกำหนดส่ง · คอลัมน์ "เส้นทางงาน" = แถบแบ่งช่วงย้อมสีตามสถานะ (เขียว/ส้ม/แดง/เทา) + "x/y ช่วง" (09-02)
...
- **ใบผลิต `/production/[id]` = แบบฟอร์มเหมือนหน้าออเดอร์ (เบสเคาะ 2026-09-08 “โอเคทำเลย” หลังหน้าลอง `/proto/work-order-form` 11 รอบ):** หัวใบ + ราง 1 2 3 (OrderStatusBar ตัวเดียวกัน · ขั้นเรียงตาม sortOrder · ยืนที่ขั้นแรกที่ยังไม่ปิด) + ปุ่มหลักของขั้นที่ยืนอยู่บนหัวใบ (ติ๊กเช็คลิสต์ครบถึงกดได้ — รอ A9.2) + เมนู ⋯ · 2 แท็บ: ขั้นตอน (ซ้าย = ตารางรายตัวแถวละไซซ์ กรอกยอดต่อแถว — รอ A9.3 · ขวา = เช็คลิสต์ + ข้อมูลออเดอร์) · สินค้า (OrderItemsDisplay ไม่มีเงิน + ลาย/ม็อกอัพ + วัตถุดิบ) · **ทุกขั้นปิดด้วยปุ่ม รวมขั้นที่ A5 เคยให้จดบนกระดาษ** (ถอยจาก A5 บางส่วน — เบสเลือกเอง) · ไม่มีคำอธิบายในจอ (A8) · ช่องคู่ = flag ในสูตร (A9.5) · ย้อนขั้น = คำสั่ง server (A9.4)
