...
- **ถอยสถานะ** (เบสเคาะ 2026-09-18): ถอยได้ทีละขั้น ไม่ข้าม (ยกเว้น `SHIPPED → QUALITY_CHECK` สำหรับของตีกลับ) และลึกสุดที่ `PRODUCTION_QUEUE` — ย้อนกว่านั้นใช้ใบแก้ไขออเดอร์ ไม่ใช่ถอยสถานะ. ในโซนโรงงาน (คิวผลิต ↔ ผลิต ↔ QC ↔ แพ็ก) ถอยได้ตามสิทธิ์เดินสถานะผลิตโดยไม่ต้องมีเหตุผล; ข้ามเส้น "ของออกจากโรงงาน" (`READY_TO_SHIP` · `SHIPPED` · `COMPLETED`) ต้อง `supervise_operations` + เหตุผลบันทึกในประวัติ (`isRollbackTransition`) และ `READY_TO_SHIP → PACKING` ถูกปฏิเสธเมื่อมีใบส่งสถานะ `SHIPPED`/`DELIVERED` — ตีกลับใบส่งก่อน. ขั้นผลิตที่ปิดด้วยหลักฐาน (เบิก/ตรวจรับ/รอบพิมพ์/ร้านนอก) ย้อนที่หลักฐาน ไม่ใช่ที่สถานะ; การถอยไม่ลบเอกสารเงินที่ออกแล้ว
- **ประวัติรายการต้องย้อนอ่านได้** (งานแก้/เคลม ก้อน 0 — เบสสั่ง 2026-09-18): ทุกประตูที่แทนรายการทั้งชุด (`saveForm` · `updateItems` · `applyChangeOrder`) ลบแถวเดิมแล้วสร้างใหม่ จึงต้องถ่ายภาพรายการเดิมด้วย `snapshotOrderItems()` (`src/lib/order-item-snapshot.ts`) เก็บลง `OrderRevision.oldValue` ก่อน `deleteMany` เสมอ — ไซซ์/สี/จำนวน/ลาย ต้องอ่านย้อนได้หลังส่งของแล้ว (เก็บเฉพาะฝั่งก่อนแก้ สภาพหลังแก้อ่านจากตัวออเดอร์) · DTO ตัด `oldValue/newValue` ให้ role ที่ไม่เห็นเงินตามนโยบายเดิม
- **ตีกลับใบส่งต้องมีเหตุผล**: `delivery.updateStatus` ไป `RETURNED` บังคับ `reason` และเขียน `OrderRevision` ชนิด `DELIVERY` พร้อมส่งเหตุผลไปกับกระดิ่งที่เตือนผู้จัดการ — ของที่ออกจากร้านแล้วกลับมาต้องเหลือร่องรอยว่าทำไม ไม่ใช่เปลี่ยนสถานะเงียบ
- **หน้าลูกค้าตอนมีรอบแก้ต้องพูดเอง** (งานแก้/เคลม ก้อน 1): ลิงก์ติดตามงานเปิดได้โดยไม่ต้องล็อกอิน และถูกส่งต่อได้ ใบเคลมจึงออกไปได้เฉพาะรูปข้อความสำเร็จรูปจาก `describeReworkForCustomer()` (`src/lib/claim-customer.ts`) เท่านั้น — ห้ามส่ง `title`/`detail`/`fault`/`faultNote`/`agreed*`/`claimNumber` ออก public ไม่ว่าทางใด · ใบที่ `CLOSED`/`CANCELLED` ต้องหายจากหน้าลูกค้าทันที · ระหว่างรอบแก้ หน้านั้นแสดงแถบ รับเรื่อง → กำลังแก้งาน → ส่งกลับให้ใหม่ แทนเส้นทางงานปกติ เพราะสถานะออเดอร์ถอยหลังจริงตอนสั่งงานแก้
...
- **รายการออเดอร์** `/orders`: เส้นสถานะกดกรอง (ป้ายเลยกำหนดต่อสถานะ) · ตัวกรองวันที่เปิดเป็นปฏิทิน+ช่วงสำเร็จรูป (ค่า `from/to` YYYY-MM-DD ใน URL) · ตาราง เลขออเดอร์/ลูกค้า/จำนวน/สถานะ/คนทำ/ต้องจัดการ/ยอด·ชำระ/กำหนดส่ง · คนทำ = ร้านนอกหรือช่างของขั้นผลิตที่เปิดอยู่ ถ้ายังไม่เข้าผลิต = ผู้เปิดออเดอร์ · ยอดเงินตามสิทธิ์ `see_order_money`
...
- **outsource** ผูกขั้นผลิต → OutsourceOrder (ล็อกแถว) SENT → RECEIVED_BACK → ตรวจรับ (ก่อน QC สุดท้าย) · เจ้าหน้าที่คุยร้านผ่าน LINE ด้วยลิงก์ `/job` · cron overdue แจ้งหัวหน้า (supervise_operations) เมื่อใบที่อยู่ที่ร้านเลยนัดรับตามวันไทย ครั้งเดียวต่อใบ
- **ตัวชี้วัดผลิต** (อ่านอย่างเดียว ไม่มีเงิน): งวดเดือนไทย ย้อน 6 เดือน · ส่งตรงเวลา = Delivery.shippedAt ล่าสุดเทียบ deadline ตามวันไทย ไม่นับยกเลิก/ไม่มี deadline/ยังไม่ส่ง · ทำถูกครั้งแรก = ΣqtyGood ÷ (ΣqtyGood+ΣqtyDefect) ของ QcRecord ในงวด · ของเสียตาม reason/ไซซ์ · เวลาต่อขั้นนับเฉพาะขั้นที่มี startedAt · ไม่มีข้อมูล = null (ไม่หารศูนย์) · สูตรใน `services/production-metrics.ts` + test
...
- **goods receipt + รอบพิมพ์ DTF (ฟิล์ม FR-)** (คลังฟิล์มคงข้อมูล/router ไว้ ไม่มีหน้าในเมนู — เบส 09-16 พิมพ์เผื่อไม่ค่อยมี) · `issueMaterials` atomic + อ่าน MaterialUsage กลับได้ · สินค้า soft-delete (`deletedAt`) เก็บประวัติ
...
- **บ้านละหน้าที่**: `/production` คิวหัวหน้า; `/production/[id]` ดูและลงมือตามสิทธิ์; `/production/print-runs` พิมพ์ DTF (ติ๊กหลายใบ กดพิมพ์เสร็จครั้งเดียว = คำสั่งรอบพิมพ์เดิมต่อกัน · ใบ V2 ไม่รับ); `/production/outsource` ใบส่งร้าน (ปุ่มจาก `availableCommands` · รับกลับผ่านใบตรวจรับ); `/production/metrics` ตัวชี้วัดอ่านอย่างเดียว; `/production/floor` งานของพนักงาน; `/factory` TV อ่านอย่างเดียว. ปุ่ม/สิทธิ์ใช้ controller และ readiness เดิม; DTF ผ่าน batch, handoff ผู้ใช้กดเอง, scan เปิด context เท่านั้น. Order/My Tasks ใช้ summary และ deep link ไปงานจริง
...
- ปุ่มลงมืออยู่ใกล้ข้อมูลที่เปลี่ยน (A14); หากยังไปต่อไม่ได้ต้องเห็นเหตุและทางไปทำสิ่งที่ขาด. เมนูรองเก็บงานที่ใช้น้อยและคงทางพักขั้น/ดูประวัติ; ย้อนกลับเป็นปุ่มจริงบนหัวใบเมื่อย้อนได้จริง และคงเป็นรายการจางพร้อมเหตุผลในเมนูรองเมื่อย้อนไม่ได้ (เบสสั่ง 2026-09-18 "กดได้ง่ายๆ"); ส่งเข้า QC ได้เมื่อทุกขั้นปิดครบ. `pairWithPrevious` คงการจับคู่จากสูตรและกล่องเปิดใบ; `production.reopenStep` เฉพาะหัวหน้า ขั้นปิดด้วยปุ่ม และขั้นถัดไปยังไม่เริ่ม
