...
- [x] **ใบผลิตเป็น job traveler เดียว** — มีทางกลับคิวเพียงจุดเดียวและไม่วาด breadcrumb/local module nav ที่ชวนออกจากใบงาน · หัวใบสรุปเลขงาน/ลูกค้า/กำหนด/จำนวน/ความคืบหน้า แล้ว ERP แบ่งเนื้อหาในใบเป็น `ทำงาน / เบิกของ / ขั้นตอนทั้งหมด` โดย default เห็น action, blocker และแบบ/จำนวนร่วมกัน · งานเบิกเสื้อปัจจุบันลงมือในแท็บทำงานโดยไม่ซ้ำ mutation, เสื้อ/วัตถุดิบส่วนรองอยู่แท็บเบิกของ, ประวัติขั้นเป็น read-only และแท็บเก็บ state ใน URL + lazy ครั้งแรก/keep-mounted หลังเปิด · Station variant คงหน้าเส้นตรงเฉพาะสถานีปัจจุบัน ไม่แสดง content tabs, MaterialUsage หรือเงิน
