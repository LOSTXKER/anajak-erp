...
- [x] **ชุดสีเดียวทั้งระบบทั้งสองธีม** — ล็อกน้ำเงินแบรนด์ `#3973b2` ไว้ · บันไดความลึก (แก้ 2026-08-26 UI-2026 เฟส 6-7): **ผืนงาน (bg) เป็นพื้นจม การ์ดลอยเหนือมันเสมอทั้งสองธีม** ส่วนกรอบเว็บวางคนละฝั่งโดยตั้งใจ — Light `chrome #ffffff` = การ์ด ลอยเหนือผืนงาน `#f1f2f4` · Dark `chrome #0a0a0b` จมใต้ผืนงาน `#0e0e10` < การ์ด `#161618` ห้ามดำสนิท · ของที่กดได้มีคู่ interaction สามชุดตามพื้นที่ไปยืน (การ์ด / กรอบเว็บ / ผืนงาน) ใช้ผิดคู่ = ชี้แล้วจอไม่ขยับ · panel ใช้เส้น 1px, มุม 8px และไม่มีเงาตกแต่ง; overlay เท่านั้นที่ลอยด้วยเงา · semantic text ผ่าน WCAG AA · public/print ไม่ถูกธีมหลังบ้านรบกวน
...
- [x] **ตัวกรองธรรมดาแบบ minimal** — ตัวเลือกไม่เกิน 5 ตัวใช้ `FilterChip` กลางเป็นข้อความ neutral ไม่มีกรอบกล่อง พื้นสี เงา หรือ radius · selected ใช้เส้นใต้และน้ำหนักข้อความพร้อม `aria-pressed` · ตัวเลือกเกิน 5 ใช้ `Select` และหลายเงื่อนไขรวมใน `FilterPopover` ที่มี label/action hierarchy เดียวกัน · มือถือยังแตะได้อย่างน้อย 44px
...
- [x] **พื้นผิวแยกตามหน้าที่ ไม่กลืนกัน** — `Section` ที่เป็นกลุ่มหลักและ `DataTable` อยู่ใน panel เดียวกับเนื้อหาที่สัมพันธ์กัน — **ตารางระดับบนสุดมีกล่องครอบเสมอ** (เบสกลับคำตัดสินใจ 2026-08-26 หลังเห็นของจริง · prop `flush` ถูกถอดออกจากระบบแล้ว); หัว panel/หัวตาราง/แถวใช้ divider ต่อเนื่องและไม่ย้อม hover รายช่อง · panel เป็นขาว/neutral dark พร้อม hairline edgeไร้เงา · field/toolbar/secondary action ใช้พื้น panel+ขอบบาง · overlay มี elevation สูงสุด · focus/error/selected ต้องชนะสถานะปกติอย่างชัดเจน
