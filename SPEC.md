...
- [x] **จุดโฟกัสชัดใน 3 วินาที** — งานเสี่ยง/งานค้างและ action หลักมาก่อนสถิติสะสม · desktop Sidebar แสดงทุกโมดูลที่มีสิทธิ์เป็นหมวดแบบแบน ไม่มีพื้น/สีครอบหมวด และใช้น้ำเงินเฉพาะ active item โดยไม่ซ่อนใน disclosure; ปุ่มหุบ/กางใช้สัญลักษณ์แผงซ้ายเปิด/ปิด โดยตอนกางปุ่มหุบอยู่ด้านขวาภายใน brand header และตอนหุบปุ่มกางย้ายไป action slot 56px แรกของ topbar เพื่อคงตราเป็นลิงก์แยก · visual, พื้นที่กดมาตรฐาน 36px (coarse pointer 44px), hover และ focus เป็นปุ่มชิ้นเดียวกัน · ไม่คาบเส้น ไม่ทับตรา/ช่องค้นหา และคง node/focus/ARIA เดิม · ส่วนมือถือคงงานหลัก 4 รายการและเปิดที่เหลือผ่าน “เพิ่มเติม”
...
- [x] **รายการออเดอร์เริ่มจากงาน ไม่เริ่มจากแผงสถานะ** — มือถือเห็นรายการหรือ empty/error ที่ลงมือทำได้ก่อน `y=600` · จอกว้างเห็น status rail ครบ · search/filter/sort/pagination/CSV ยังใช้งานจริง · ในแต่ละรายการทั้ง desktop/mobile ระบุตัวใบด้วย **เลขที่ออเดอร์ + ชื่อลูกค้า** เท่านั้น ไม่มีบรรทัดรองอื่น
- [x] **ไม่มีระบบชื่องาน** (เบสสั่ง 2026-08-30 "เอาระบบชื่องานออกให้หมด") — ทั้งออเดอร์และใบเสนอราคาไม่มีช่องชื่องานให้กรอก ไม่มีการตั้งชื่ออัตโนมัติ และไม่แสดงชื่องานที่ไหนเลย (หน้าเว็บ · เอกสารพิมพ์ · หน้าลูกค้า · กระดิ่ง · ค้นหา) · ใบงานอ้างด้วยเลขที่ + ลูกค้า · สิ่งที่บอกว่า "ทำอะไร" คือ **ชื่อรายการงาน** (`OrderItem.description`) ซึ่งยังอยู่ครบ · คอลัมน์ `orders.title` / `quotations.title` ถูกลบถาวรจากฐานข้อมูลแล้ว (migration `20260830120000_drop_job_title`)
