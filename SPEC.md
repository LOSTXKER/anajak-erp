...
- [x] **บ้านละหน้าที่**: `/production` รายการทุกงาน (server-side pagination) · `/production/[id]` Control Record ของหัวหน้า (release/priority/assignment/exception/audit · ไม่มี start/report/complete แทนพนักงาน) · `/factory/station` พนักงานลงมือ (งานปัจจุบัน + primary action เดียว · DTF เป็น batch · handoff ผู้ใช้กดเอง · scan เปิด context เท่านั้น) · `/factory` TV อ่านอย่างเดียว · Order/My Tasks = summary + deep link · route เก่า (`print-runs` `films` `/outsource`) redirect เข้าบ้าน canonical
...
- [ ] **PV2.8 cutover** — เบส walkthrough บทบาทหัวหน้า + พนักงานครบ flow บนฐาน demo → seed routing + work center ฐานจริง (additive) → backup → เปิด `PRODUCTION_V2_ENABLED` บน production ทีละขั้น (เบสอนุมัติทุกขั้น) → ลบ legacy UI + writer เก่า
  - ข้อควรรู้จากการซ้อม 2026-09-01: รายงานผลผลิตต้องแยกตาม quantity line (ยิงยอดรวมถูกปฏิเสธ) · ขั้น DTF เริ่มจากรอบพิมพ์ที่ผูกหลักฐาน ไม่เริ่มลอย ๆ → การซ้อมที่เหลือต้องกดในจอจริง `/factory/station` ไม่ใช่ยิงสคริปต์
...
- [x] **`/production`**: ตัวกรองเป็นสายพานขั้นงาน (ตัวเลขนำ ชื่อรอง + ลูกศร + ไอคอนรถบนขั้นร้านนอก · เก็บใน URL `?station=`) · ไม่มีช่องเรียงบนคอม · ตารางแบ่งกลุ่มตามกำหนดส่ง · คอลัมน์ "เส้นทางงาน" = แถบแบ่งช่วงย้อมสีตามสถานะ (เขียว/ส้ม/แดง/เทา) + "x/y ช่วง" (09-02)
