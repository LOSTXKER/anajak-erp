...
1. เริ่มงาน → อ่าน `SPEC.md` + `ROADMAP.md` (ไฟล์แผนของ repo นี้) ก่อนแตะโค้ด · `PROGRESS.md` hook โหลดให้ · งานไม่อยู่ใน ROADMAP = ถามเบสก่อน
2. งานใหญ่ → อัปเดต `ROADMAP.md` ก่อน (แตก task) · ทำทีละ task
...
4. ก่อนจบงาน → เขียนทับ `PROGRESS.md` (ทำอะไร·ค้าง·NEXT) + ติ๊ก `ROADMAP.md` → commit (บรรทัดแรก = ประโยคไทยที่คนอ่านออก ≤90 ตัว) → push เป็น branch
