...
### Enterprise Modules (Phase 2)
25. [Human Resources (HRM)](#human-resources-hrm)
26. [Accounting & Finance](#accounting--finance)
27. [Advanced WMS](#advanced-wms)
28. [Asset & Maintenance](#asset--maintenance)
29. [CRM & Sales Pipeline](#crm--sales-pipeline)

### Smart Factory Upgrades (Phase 3) ⭐ NEW
30. [Visual Scheduler (Gantt)](#visual-scheduler-gantt--capacity)
31. [Shop Floor Kiosk](#shop-floor-kiosk-tablet-app)
32. [Fleet & Route Management](#fleet--route-management)

### Technical
33. [Database Schema Updates](#database-schema-updates)
34. [Implementation Roadmap](#implementation-roadmap)
35. [Technology Stack & Design System](#technology-stack--design-system)
...
## 🏢 Enterprise Modules (Phase 2 Expansion)

### Human Resources (HRM)

### ระบบบริหารจัดการพนักงานและค่าแรง (เน้นโรงงาน)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    HRM & PAYROLL WORKFLOW                                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   INPUTS                                           CALCULATION                  │
│   ┌───────────────────────────┐         ┌──────────────────────────────────┐    │
│   │  👥 Employee Profile      │         │  💰 Payroll Engine               │    │
│   │  • Info & Contracts       │         │  1. Base Salary / Day Rate       │    │
│   │  • Base Salary            │         │  +  OT (Time Attendance)         │    │
│   │  • Skill Matrix           │───────► │  +  Piece Rate (Production Log)  │    │
│   │  └────────────────────────┘         │  +  Incentives / Bonus           │    │
│   │                                     │  -  Deductions (Tax/SSO/Late)    │    │
│   │  ┌───────────────────────────┐      │                                  │    │
│   │  │  ⏰ Time Attendance       │      │                                  │    │
│   │  │  • Clock In/Out         │───────►│  =  NET SALARY                   │    │
│   │  │  • OT Requests          │        └─────────────────┬────────────────┘    │
│   │  └───────────────────────────┘                        │                     │
│   │                                                       ▼                     │
│   │  ┌───────────────────────────┐             ┌────────────────────┐           │
│   │  │  🧵 Production Logs       │             │  📄 Payslip Gen    │           │
│   │  │  • Job Completed (Qty)  │───────►       │  🏦 Bank File      │           │
│   │  │  • QC Pass/Fail         │               └────────────────────┘           │
│   │  └───────────────────────────┘                                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
...
#### Skill Matrix (ตารางทักษะ)
ใช้สำหรับ Auto-assign งาน และกำหนดเรทค่าแรง

| Employee | Position | DTF Print | Sewing | Packing | QC | Hourly Rate | Piece Rate |
|----------|----------|-----------|--------|---------|----|-------------|------------|
| Emp A    | Printer  | ⭐⭐⭐⭐⭐ | ❌ | ⭐⭐⭐ | ⭐⭐ | ฿60 | ฿2/print |
| Emp B    | Sewer    | ❌ | ⭐⭐⭐⭐⭐ | ⭐ | ⭐⭐ | ฿80 | ฿15/sew |
| Emp C    | General  | ⭐⭐ | ❌ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ฿45 | ฿1/pack |

---

### Accounting & Finance

### ระบบบัญชีและการเงิน (เชื่อมโยงทั้งระบบ)
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   [SALES CYCLE]                        [PURCHASE CYCLE]                         │
│                                                                                 │
│   Customer                             Supplier                                 │
│      │ (Order)                            ▲ (PO)                                │
│      ▼                                    │                                     │
│   ┌─────────────┐                      ┌─────────────┐                          │
│   │ Sale Order  │                      │ Purchase Ord│                          │
│   └──────┬──────┘                      └──────┬──────┘                          │
│          │                                    │                                 │
│          ▼ (Invoice)                          ▼ (Bill)                          │
│   ┌─────────────┐                      ┌─────────────┐                          │
...
│   │  • Tax Report (รายงานภาษีซื้อ/ขาย)                    │                      │
│   └──────────────────────────────────────────────────────┘                      │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

#### Job Costing (การคำนวณต้นทุนจริงราย Order)
*   **Direct Material:** ค่าผ้า + ค่าหมึก + ค่า Addon (ดึงจาก Stock FIFO)
*   **Direct Labor:** ค่าแรงรายชิ้น (จาก HRM) หรือ Time-based allocation
*   **Overhead:** ค่าไฟ + ค่าเสื่อมเครื่องจักร (Allocate ตาม Machine Hours)
*   **Result:** รู้กำไรจริง (Real Profit) ต่อ Order ไม่ใช่แค่กำไรประเมิน

---

### Advanced WMS

### ระบบจัดการคลังสินค้าขั้นสูง (Inventory Operations)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    INVENTORY MOVEMENT FLOW (INBOUND & OUTBOUND)                 │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│   📥 INBOUND (รับสินค้าเข้า)                                                    │
│   ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐│
│   │  🚚 Receive  │───► │  📝 Inspect  │───► │  🏷️ Labeling │───► │  🏗️ Put-away ││
│   │  (จาก PO)     │     │  (QC Check)  │     │  (ติด QR)     │     │  (เข้าชั้น)   ││
│   └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘│
│          ▲                    │ Fail               │                    │        │
│          │                    ▼                    │                    │        │
...
│                                                                                 │
│   📤 OUTBOUND (เบิกสินค้าออก)                                                   │
│   ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     ┌──────────────┐│
│   │  📋 Request  │───► │  🔍 Picking  │───► │  ✅ Verify   │───► │  ✂️ Issue    ││
│   │  (ใบเบิก)      │     │  (หยิบของ)    │     │  (ตรวจก่อนส่ง) │     │  (ตัดสต็อก)  ││
│   └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘│
│          ▲                    │                    │                    │        │
│          │                    │                    │                    │        │
│      Production               │                    │                    ▼        │
│      (Job Ticket)             ▼                    ▼           [ STOCK DEDUCTED - ]│
│                        [ Suggested Bin ]    [ Handover Sig ]                    │
│                        (ระบบบอกตำแหน่ง)       (ลายเซ็นผู้รับ)                       │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  🧵 FABRIC ROLL MANAGEMENT (จัดการผ้าเป็นม้วน)                                   │
├─────────────────────────────────────────────────────────────────────────────────┤
│  Roll ID: FR-2024-001   Type: Cotton 32   Color: White                          │
│                                                                                 │
│  [ Initial Weight: 20.0 kg ] ──────────────────────────────────────────┐        │
│                                                                        │        │
│      Usage Log:                                                        │        │
│      • Job #101: Cut 5.0 kg (Est. 20 yards) -> Weight Left: 15.0 kg    │        │
│      • Job #102: Cut 2.5 kg (Est. 10 yards) -> Weight Left: 12.5 kg    │        │
│      • Job #103: Defect 0.5 kg              -> Weight Left: 12.0 kg    │        │
│                                                                        │        │
│  [ Current Status: 12.0 kg ] <─────────────────────────────────────────┘        │
│         │                                                                       │
│         ▼                                                                       │
│  [ Alert: Low Stock ] (Re-order point at 10kg)                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```

#### Warehouse Features
*   **Bin Location:** ระบุตำแหน่งเก็บละเอียด (Zone A > Rack 1 > Shelf 3)
*   **Unit Conversion:** ซื้อเป็น Kg -> เบิกเป็น Yard/Meter -> ตัดเป็น Pcs
*   **Dead Stock Alert:** แจ้งเตือนผ้าที่เก็บนานเกิน 6 เดือน (เสี่ยงสีเพี้ยน/ขึ้นรา)
*   **QR Code Move:** ย้ายของในโกดังด้วยการสแกน QR Code (Move Stock)
*   **Stock Adjustment:** ปรับปรุงยอดสต็อก (Ad-hoc) กรณีของหายหรือชำรุด พร้อมระบุเหตุผล

---

### Asset & Maintenance

### ระบบบริหารเครื่องจักรและการซ่อมบำรุง

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  🛠️ MACHINE LIFECYCLE MANAGEMENT                                               │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌──────────────┐      ┌──────────────┐      ┌──────────────┐                   │
│  │ 🟢 Running   │      │ 🟡 Warning   │      │ 🔴 Breakdown │                   │
│  │ (Active)     │      │ (PM Due)     │      │ (Downtime)   │                   │
│  └──────┬───────┘      └──────┬───────┘      └──────┬───────┘                   │
│         │                     │                     │                           │
│         │ Usage Count         │ Schedule            │ Issue Report              │
│         ▼                     ▼                     ▼                           │
│  ┌──────────────────────────────────────────────────────────┐                   │
│  │                   MAINTENANCE MODULE                     │                   │
│  │  • Registry: ทะเบียนเครื่องจักร (Serial, Buy Date)        │                   │
│  │  • PM Schedule: แผนซ่อมบำรุงตามระยะ (ทุก 10,000 prints)   │                   │
│  │  • Spare Parts: สต็อกอะไหล่ (หัวพิมพ์, สายพาน)            │                   │
│  │  • History: ประวัติการซ่อมและค่าใช้จ่าย                     │                   │
│  └──────────────────────────────────────────────────────────┘                   │
│         │                                                                       │
│         ▼                                                                       │
│  🚦 Impact to Production:                                                       │
│     Machine Down -> ⛔ Block Scheduling -> ⚠️ Alert Planner                     │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
### ระบบบริหารงานขายและความสัมพันธ์ลูกค้า
...
│                                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │  📥 LEADS    │  │  💬 TALKING  │  │  📝 QUOTED   │  │  ✅ WON      │         │
│  ├──────────────┤  ├──────────────┤  ├──────────────┤  ├──────────────┤         │
│  │ Interest New │  │ Reqs Clear   │  │ Sent Price   │  │ PO Received  │         │
│  │ [Customer A] │  │ [Customer B] │  │ [Customer C] │  │ [Customer D] │         │
│  │ Value: ???   │  │ Value: 5k    │  │ Value: 20k   │  │ Value: 50k   │         │
│  │ 2 days ago   │  │ Today        │  │ Exp: 3 days  │  │ -> Order     │         │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘         │
│                                                                                 │
│  📊 Sales Performance                                                           │
│  • Target vs Actual (รายคน/รายทีม)                                               │
│  • Conversion Rate (Lead -> Won)                                                │
│  • Lost Reason Analysis (ราคาแพง / ส่งช้า / คู่แข่ง)                              │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
## 🚀 Smart Factory Upgrades (Phase 3)

### 30. Visual Scheduler (Gantt & Capacity)

### ระบบวางแผนการผลิตแบบ Visual (ลาก-วาง)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    PRODUCTION GANTT CHART VIEW                                  │
├─────────────────────────────────────────────────────────────────────────────────┤
│  📅 DEC 2024      │ 09 Mon    │ 10 Tue    │ 11 Wed    │ 12 Thu    │ 13 Fri      │
│  ─────────────────┼───────────┼───────────┼───────────┼───────────┼─────────────┤
│  🖨️ DTF Station 1 │ [JOB-101] │ [JOB-102] │           │ [JOB-105] │             │
│  (Cap: 200/day)   │ 100% Full │ 80% Load  │  🟢 Free  │ 50% Load  │             │
│  ─────────────────┼───────────┼───────────┼───────────┼───────────┼─────────────┤
│  🖨️ DTF Station 2 │ [JOB-103] │ [  Maintenance  ]     │ [JOB-106] │             │
│  (Cap: 200/day)   │ 100% Full │ 🔧 Down   │           │ 40% Load  │             │
│  ─────────────────┼───────────┼───────────┼───────────┼───────────┼─────────────┤
│  🧵 Sewing Line A │           │ [JOB-101] ──────────► │ [JOB-102] │             │
│  (Cap: 100/day)   │           │ (Wait DTF)│           │           │             │
│  ─────────────────┼───────────┼───────────┼───────────┼───────────┼─────────────┤
│                                                                                 │
│  [📥 Unassigned Jobs]: [JOB-107 (Due Dec 15)] [JOB-108 (Due Dec 20)]            │
│  👉 Drag & Drop jobs here to assign                                             │
└─────────────────────────────────────────────────────────────────────────────────┘
```

#### Features
*   **Drag & Drop Scheduling:** ลากงานลงตาราง แล้วระบบคำนวณเวลาเสร็จให้อัตโนมัติ
*   **Conflict Alert:** แจ้งเตือนถ้าเครื่องเต็ม หรือวัตถุดิบมาไม่ทัน (Link กับ WMS)
*   **Auto-Optimization:** ปุ่ม "Magic Sort" จัดเรียงงานให้เต็มประสิทธิภาพที่สุด (ลด Downtime)

---

### 31. Shop Floor Kiosk (Tablet App)

### หน้าจอสำหรับพนักงานผลิต (ลดความผิดพลาด)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  📱 WORKER TABLET INTERFACE (KIOSK MODE)                                        │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  JOB: #ORD-2024-0123 / ITEM-01                                                  │
│  Product: เสื้อยืด Cotton 32 สีดำ Size L                                          │
│  Target: 50 ตัว                                                                  │
│                                                                                 │
│  ┌─────────────────────────────────────┐  ┌──────────────────────────────────┐  │
│  │                                     │  │  STATUS: 🟢 RUNNING              │  │
│  │         [ SHOW MOCKUP ]             │  │                                  │  │
│  │      (รูปตำแหน่งสกรีนชัดๆ)             │  │  ✅ Finished:  [ 15 ] / 50     │  │
│  │                                     │  │  ❌ Defect:    [  1 ]          │  │
│  │                                     │  │                                  │  │
│  │                                     │  │  [➕ Count +1]  [➖ Undo]      │  │
│  │                                     │  │                                  │  │
│  └─────────────────────────────────────┘  └──────────────────────────────────┘  │
│                                                                                 │
│  [⏸️ PAUSE]  [⚠️ REPORT ISSUE]  [🏁 COMPLETE JOB]                               │
│                                                                                 │
│  📝 Note: ระวังสีสกรีนทับตะเข็บ                                                    │
└─────────────────────────────────────────────────────────────────────────────────┘
```

#### Features
*   **Big Buttons:** ปุ่มใหญ่ กดง่าย ใส่ถุงมือก็กดได้
*   **Digital Job Ticket:** ไม่ต้องปริ้นใบสั่งงาน (ลดกระดาษ) เห็น Mockup ชัดเจน
*   **Real-time Tracking:** ผู้บริหารเห็นยอดผลิตวิ่งขึ้นทันทีที่หน้าจอ Dashboard

---

### 32. Fleet & Route Management

### ระบบจัดการรถส่งของและเส้นทาง (สำหรับรถบริษัท)

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    DELIVERY ROUTE PLANNER                                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│  📅 Date: 2024-12-10    🚗 Vehicle: Toyota Hilux (ตฮ-9999)    Drivers: นาย สมชาย │
│                                                                                 │
│  📍 Warehouse (Start: 09:00)                                                    │
│      │                                                                          │
│      ├── 📦 Order A (เขตจตุจักร) - 10:00 ✅ Delivered                           │
│      │                                                                          │
│      ├── 📦 Order B (เขตลาดพร้าว) - 10:45 ⏳ On the way                         │
│      │                                                                          │
│      ├── 📦 Order C (เขตบางกะปิ) - 11:30                                        │
│      │                                                                          │
│      └── 🏠 Return to Base - 12:30                                              │
│                                                                                 │
│  🗺️ [View Map]  [🖨️ Print Manifest]  [📲 Send to Driver App]                    │
└─────────────────────────────────────────────────────────────────────────────────┘
```

#### Features
*   **Zone Grouping:** จัดกลุ่มออเดอร์ตามเขตให้อัตโนมัติ
*   **Driver App:** คนขับมีแอปดูแผนที่ + ให้ลูกค้าเซ็นรับงานบนมือถือ (e-Signature)
*   **Proof of Delivery:** ถ่ายรูปตอนส่งของ + พิกัด GPS เป็นหลักฐาน

---

## 33. 🗄️ Database Schema Updates
...
### Phase 7: Enterprise Expansion (Month 4+)
- [ ] HRM: Employee DB & Skill Matrix
- [ ] HRM: Piece Rate Logic
- [ ] Finance: GL & Cost Accounting
- [ ] WMS: Fabric Roll Tracking
- [ ] Asset: Machine Maintenance
- [ ] CRM: Sales Pipeline

### Phase 8: Smart Factory (Month 6+)
- [ ] Visual Scheduler (Gantt)
- [ ] Shop Floor Tablet App
- [ ] Delivery Route Planner
- [ ] IoT Integration (Machine Counters)

---

---

## 35. 🛠️ Technology Stack & Design System

### Technology Stack (Modern Web 2025)

เราเลือกใช้ Stack ที่มีความทันสมัย ประสิทธิภาพสูง และดูแลรักษาง่าย (Developer Friendly)

#### Frontend (Web & PWA)
*   **Framework:** [Next.js 14+](https://nextjs.org) (App Router) - เร็ว, SEO ดี, โครงสร้างชัดเจน
*   **Language:** [TypeScript](https://www.typescriptlang.org) - ลดบั๊ก Type Error, Code แข็งแรง
*   **Styling:** [Tailwind CSS](https://tailwindcss.com) - เขียน UI เร็ว, ปรับแต่งง่าย
*   **Components:** [Shadcn/UI](https://ui.shadcn.com) - สวย, Modern, Accessible (Radix UI base)
...
*   **Platform:** [Supabase](https://supabase.com) (Firebase Alternative แต่ดีกว่าสำหรับ Relational Data)
*   **Database:** PostgreSQL (Relational DB ที่ดีที่สุด)
*   **Auth:** Supabase Auth (Email, Social, Phone) + RLS (Row Level Security)
*   **Real-time:** Supabase Realtime (สำหรับ Kiosk, Status Update)
*   **Storage:** Supabase Storage (เก็บรูป Mockup, สลิป)
*   **Functions:** Supabase Edge Functions (Deno) สำหรับ Logic ซับซ้อน

#### DevOps & Deployment
*   **Hosting:** [Vercel](https://vercel.com) (Best for Next.js)
...
เน้นความ "Clean, Contextual, & Efficient" ลดความซับซ้อนของข้อมูล ให้ผู้ใช้โฟกัสสิ่งที่ต้องทำ

#### 1. Color Palette (Mood: Professional & Trust)
*   **Primary:** `Slate-900` (Dark Navy/Black) - ความเป็นมืออาชีพ
*   **Secondary:** `Blue-600` (Royal Blue) - Action Buttons, Links
*   **Success:** `Emerald-500` - Completed, Paid, Passed
*   **Warning:** `Amber-500` - Pending, In-Progress, Late risk
*   **Error:** `Rose-500` - Failed, Rejected, Overdue
*   **Background:** `Slate-50` (Off-white) - สบายตา อ่านง่ายกว่าขาวจั๊วะ

#### 2. Layout Strategy
*   **Admin/Backoffice (Desktop First):**
    *   Sidebar Navigation (Collapsible)
    *   Data Dense Tables (ตารางข้อมูลแน่นแต่ดูง่าย)
    *   Dashboard Widgets
*   **Sales/Driver/Customer (Mobile First):**
    *   Bottom Tab Navigation
    *   Card Views (แทนตาราง)
    *   Large Touch Targets (ปุ่มใหญ่)

#### 3. Key UX Patterns
*   **Wizards:** แบ่ง Flow ยาวๆ (เช่น สร้าง Order) เป็นขั้นตอนย่อย (Step 1, 2, 3)
*   **Drawers/Modals:** แก้ไขข้อมูลโดยไม่ต้องเปลี่ยนหน้า (Context ไม่หาย)
*   **Optimistic UI:** กดปุ่มแล้ว UI เปลี่ยนทันทีไม่ต้องรอ Server (รู้สึกว่าแอปเร็วมาก)
*   **Visual Status:** ใช้ Badges/Icons แทนข้อความสถานะยาวๆ
