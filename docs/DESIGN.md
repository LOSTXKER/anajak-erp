...
- `/production/[id]` ย่อ **หัวใบ + สรุปใบงาน** ให้เหลือบริบทที่จำเป็น แล้วใช้ process bar แบบจุดเชื่อมจาก workflow จริง: รางเป็นเส้น neutral เต็มความกว้าง, ขั้นเสร็จเป็น check, ขั้นอื่นเป็นเลขและชื่ออยู่ใต้ node; workflow tone ต้องไม่ผูกกับ selection ring · ค่าเริ่มต้นเปิดหัวคิวที่ลงมือได้ ผู้ใช้สลับดูขั้นเสร็จ/ปัจจุบัน/ที่รอได้ทีละ panel โดยการเลือกเป็น view state เท่านั้นและไม่เปลี่ยนสถานะ · งานหลายเลนที่ทำพร้อมกันต้องถูกบอกตรงๆ ไม่วาดเป็นลำดับบังคับปลอม · panel ของขั้นที่เลือกมี blocker/จำนวน/primary action จาก `selectNowSteps` เดิม และกรองแบบ/วิธีพิมพ์ให้สัมพันธ์กับขั้นนั้น; ขั้นอนาคต/เสร็จแล้วอ่านอย่างเดียว · `GARMENT_PICK` ใช้การ์ดเบิกจริงเพียงจุดเดียวและห้าม generic complete · วัตถุดิบระดับทั้งใบอยู่ใน native disclosure เดียวที่ lazy จนเปิดครั้งแรก · query เก่า `?tab=inventory` ยังเปิด disclosure ส่วน `?tab=history` ลงที่ process bar ซึ่งเห็นเส้นทางงานอยู่แล้ว · Station reuse controller/mutation เดียวกันแต่คงหน้าเส้นตรงและไม่ mount process bar/disclosure ของ ERP/MaterialUsage
