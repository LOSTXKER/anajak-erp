...
- `/production/[id]` ย่อ **หัวใบ + สรุปใบงาน** ให้เหลือบริบทที่จำเป็น แล้ววาง action, blocker/ความพร้อม, เสื้อที่เกี่ยวกับงานปัจจุบัน และแบบ/จำนวนไว้ใน workspace แรกเดียวกัน · เสื้อ/วัตถุดิบที่ไม่ใช่งานปัจจุบันกับ `ProductionStepsList readOnly` อยู่ใน disclosure ชั้นรองและ lazy จนเปิดครั้งแรก · query เก่า `?tab=inventory|history` ยังเปิด disclosure ที่ตรงกันเพื่อไม่ทำลิงก์เดิมพัง · Station reuse ข้อมูลเดียวกันเป็นหน้าเส้นตรงและไม่ mount disclosure ของ ERP/MaterialUsage
