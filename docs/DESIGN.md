...
| `/production/[id]` | shared dashboard `AppShell` | job traveler แบบ single-task: action + ความพร้อม + คำสั่งงานอยู่ workspace เดียว; ข้อมูลรองเปิดดูตามจำเป็น |
...
- `/production/[id]` ย่อ **หัวใบ + สรุปใบงาน** ให้เหลือบริบทที่จำเป็น แล้วใช้ workspace surface เดียวเรียง **งานปัจจุบัน + blocker/ความพร้อม + เสื้อ/จำนวน + primary action → แบบและสเปก → ขั้นถัดไป** · desktop แบ่งสองฝั่งโดยให้งานหลักกว้างกว่าสเปก ส่วน mobile ใช้ DOM เดียวเรียง action ก่อน · `GARMENT_PICK` ที่ลงมือได้แสดงผ่านการ์ดเบิกจริงเพียงจุดเดียว แต่ blocker/read-only ยังอยู่ในรายการงานเพื่อไม่กลบเหตุผล · เสื้อ/วัตถุดิบที่ไม่ใช่งานปัจจุบันกับ `ProductionStepsList readOnly` เป็น compact embedded content ใน disclosure surface เดียวและ lazy จนเปิดครั้งแรก · query เก่า `?tab=inventory|history` ยังเปิด disclosure ที่ตรงกันเพื่อไม่ทำลิงก์เดิมพัง · Station reuse ข้อมูลเดียวกันเป็นหน้าเส้นตรงและไม่ mount disclosure ของ ERP/MaterialUsage
