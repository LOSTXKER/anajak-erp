...
**ความหนาแน่นแถวตาราง:** `DataTable.Root cellPadding` กำหนดระยะแนวนอนร่วมของหัว/เซลล์/ปุ่มเรียง: `default` 24px, `compact` 12px (ผลิต), `responsive` 16px ก่อน xl / 24px ตั้งแต่ xl (ออเดอร์); แนวตั้ง `py-4` · แถวจริงสูง **75px** — `page-skeleton.tsx` ต้องใช้ตัวเลขเดียวกัน
...
**A11 (2026-09-09):** ปรับหน้าที่มีอยู่ผ่านส่วนกลางก่อน — `PageShell` ลดระยะบนมือถือ, `StatCard loading` แยกค่าที่ยังไม่ทราบจากเลขศูนย์, `DataTable` เปิดลิงก์โดยไม่แย่ง control/การเลือกข้อความ. หน้า production บนมือถือยังเป็นตาราง โดยรวมเลขใบ/จำนวน/กำหนดส่ง/ตอนนี้ไว้เซลล์แรก; ใบผลิตแสดงภาพและชื่อสินค้าเพียงครั้งเดียวต่อกลุ่มแล้วตารางไซซ์ 4 คอลัมน์. Station กับใบผลิตใช้ผลเช็คลิสต์ที่บันทึกเดียวกันและแยก "พร้อมทำ" จาก "กำลังทำ"
...
### คำอธิบายในหน้าจอ — 3 ระดับ (เบสสั่ง 2026-09-06 "อธิบายเยอะเกินไป รก · จำเป็นค่อยเป็นไอคอน tooltip")
| ระดับ | เมื่อไหร่ | ทำยังไง |
|---|---|---|
| **ไม่ต้องบอก** | หน้าจอบอกอยู่แล้วด้วยโครงสร้าง (ช่องราคาเป็นขีด = ไม่คิดราคา · ปุ่มชื่อชัดอยู่แล้ว) · คำอธิบายว่าหน้านี้/หมวดนี้คืออะไร | ไม่มีข้อความ — `PageShell`/`PageHeader` ไม่ใส่ `description` ที่แค่ขยายชื่อหน้า · `AddCard` ไม่ต้องมี `desc` |
| **จำเป็นแต่ไม่ต้องอ่านทุกครั้ง** | กติกา/สูตร/ที่มาของค่า (นับอายุหนี้จากวันไหน · ค่านี้หักจากอะไร · ค่ามาตรฐานเท่าไร) | ไอคอน ⓘ ข้างป้าย = `help` ของ `Field` / `Section` / `PageShell` หรือ `<HelpTip>` ตรงๆ — **ไม่ใช่ประโยคใต้ช่อง (`description`) และไม่ใช่วงเล็บต่อท้ายป้าย** |
| **ต้องเห็นตลอด** | สถานะที่เปลี่ยนสิ่งที่ทำได้ตอนนี้ (ช่องล็อกเพราะอะไร · เตือนก่อนบันทึก · สิทธิ์ไม่พอ) · ข้อความรอ/ว่าง (`EmptyState`) | ข้อความสั้น 1 บรรทัดได้ — `description` ของ `Field` สงวนไว้ให้เรื่องนี้เท่านั้น |

ห้ามใส่ข้อความอธิบาย "เผื่อไว้" — ถ้าไม่แน่ใจว่าอยู่ระดับไหน = ระดับ 1 (ไม่ต้องบอก) แล้วรอคนถาม
