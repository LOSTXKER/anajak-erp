...
| `/production/[id]` | shared dashboard `AppShell` | ใบงานหน้าเครื่องพื้นสว่าง: `xl` ใช้ stage dock แนวตั้งซ้าย ส่วน 1024/mobile ใช้ selector แนวนอน; ขั้นที่เลือกเปิด work canvas โดย selector ไม่เปลี่ยนสถานะ |
...
- `/production/[id]` ฝั่ง ERP เป็น **ใบงานหน้าเครื่อง** ในโลก P1.0: หัวพื้นสว่างแบบ compact ทำให้เลขงาน/สถานะ/ลูกค้านำ utility, stage dock แนวตั้งแคบอยู่ซ้ายตั้งแต่ `xl` และเปลี่ยนเป็น selector แนวนอนที่ 1024/mobile · ขั้นเสร็จ/current/failed/waiting/queued ใช้ semantic tone ส่วน selected ใช้ Anajak blue; การเลือกขั้นเป็น manual tab/view state เท่านั้นและรองรับหลาย current โดยไม่เปลี่ยนสถานะ · ขั้นที่เลือกอยู่บน work canvas จำกัดความกว้าง โดย blocker จำนวนสินค้า/ยอดขาด และ primary action เดียวจาก `selectNowSteps` เดิมต้องอยู่ใน first viewport; แบบและสเปกเป็น reference sidecar เฉพาะขั้นและซ้อนใต้ action ก่อน `xl` · `GARMENT_PICK` ใช้การ์ดเบิกจริงเพียงจุดเดียว แสดงต้องใช้/เบิกสุทธิ/ยังขาดและห้าม generic complete · เสื้อ/วัตถุดิบระดับทั้งใบกับเส้นทางทั้งหมดอยู่ใน inspector เดียวแบบ drawer/bottom sheet; query เก่า `?tab=inventory|history` ต้องเปิด section ที่ตรงกันและปิดแล้วคืน focus ให้ trigger · Station reuse controller/mutation/permission/status/action policy เดียวกันแต่คง PageHeader/summary/หน้าเส้นตรงเดิม และไม่ mount stage dock, inspector หรือ MaterialUsage ของ ERP
