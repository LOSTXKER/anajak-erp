...
src/server/services/ ★ business logic แกน — เงิน/สถานะ/เลขเอกสาร ต้องอยู่ที่นี่
  ...
  receivables.ts    นิยามลูกหนี้/ยอดค้าง/aging/วงเงินเครดิต (exposure) + ด่าน SALES (P1)
...
- `npm run verify:bn` — integration จริงกับ DB: ใบวางบิล/aging/ด่านวงเงินเครดิต — สร้างข้อมูล [BN-VERIFY] แล้วลบเกลี้ยง + คืน DocumentSequence ⚠️ ห้ามรันบน DB ที่ใช้งานจริงแล้ว
